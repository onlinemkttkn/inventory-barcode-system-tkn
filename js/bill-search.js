...
els.voidButton.addEventListener('click', () => {
  ...
  const popup = window.open(
    url,
    'tknVoidBillPopup',
    'popup=yes,width=620,height=640,resizable=yes,scrollbars=yes'
  );
...
async function initializeBillSearch() {
  try {
    ...
    console.error('Bill search initialization error:', error);
    els.resultMessage.textContent =
      `เริ่มต้นหน้าค้นหาบิลไม่สำเร็จ: ${error.message}`;
  }
}

(() => {
  ...
  function load() {
    ...
    const invalidReceipts = S.receipts.filter((row) => !row.product_id && /ยังไม่พบในหน้าจัดการสินค้า|ตรวจสอบสินค้าไม่สำเร็จ/.test(String(row.name || "")));
    const invalidBoxItems = S.boxItems.filter((row) => !row.product_id && /ยังไม่พบในหน้าจัดการสินค้า|ตรวจสอบสินค้าไม่สำเร็จ/.test(String(row.name || "")));
    if (invalidReceipts.length) S.receipts = S.receipts.filter((row) => !invalidReceipts.includes(row));
    if (invalidBoxItems.length) S.boxItems = S.boxItems.filter((row) => !invalidBoxItems.includes(row));
    if (invalidReceipts.length || invalidBoxItems.length) {
      localStorage.setItem(KEY, JSON.stringify(S));
      setTimeout(() => msg(`นำรายการรหัสไม่ถูกต้องออกจากรอบงานแล้ว ${invalidReceipts.length + invalidBoxItems.length} รายการ`, "error"), 50);
    }
  ...
  async function cloudAudit(action, ref, detail) {
    try {
      await supabaseClient.from("container_audit_log").insert({
        ...
        detail: { message: detail || "", source: "box-qr-v5.25.3" },
        user_id: S.userId,
      });
    } catch (error) {
  ...
  function buildProductCreateUrl(raw, context = "receive") {
    const value = cleanScan(raw);
    const returnParams = new URLSearchParams({ tab: context === "box" ? "box" : "receive", scan: value, created: "1" });
  ...
  function unknownProductDialog(raw, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      if (!overlay) {
        ...
        overlay.innerHTML = `
          <div class="unknown-product-card" role="dialog" aria-modal="true" aria-labelledby="unknownProductTitle">
            ...
            <p class="unknown-product-warning">ระบบจะไม่รับเข้าและไม่เพิ่มเข้ากล่องจนกว่าจะมี SKU จริงในหน้าจัดการสินค้า</p>
        ...
        style.textContent = `
          ...
          #unknownProductOverlay[hidden]{display:none}
          .unknown-product-card{width:min(520px,100%);background:#fffaf4;border:2px solid #c8101e;border-radius:22px;padding:28px;box-shadow:0 24px 70px rgba(0,0,0,.35);text-align:center}
          ...
          .unknown-product-card h2{margin:0 0 8px;font-size:1.65rem}
          .unknown-product-card p{margin:8px 0;line-height:1.55}
          .unknown-product-card code{display:block;margin:14px 0;padding:12px;border-radius:12px;background:#241719;color:#fff;font-size:1.1rem;overflow-wrap:anywhere}
          .unknown-product-warning{color:#9f1823;font-weight:800}
          .unknown-product-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:20px}
          .unknown-product-actions button{min-height:48px;border:1px solid #d8cbc5;border-radius:12px;background:#fff;font-weight:800;cursor:pointer}
          .unknown-product-actions .primary{border-color:#c8101e;background:#c8101e;color:#fff}
      ...
      text.textContent = isLookupError
        ...
        : "รหัสที่สแกนยังไม่อยู่ในหน้าจัดการสินค้า กรุณาสร้างสินค้าและกำหนด SKU จริงก่อนรับเข้า";
  ...
  async function requireKnownProduct(product, raw, context = "receive") {
    if (product?.id) return true;
    const action = await unknownProductDialog(raw, { lookupError: product?.lookupError || "" });
    if (action === "create") {
      location.href = buildProductCreateUrl(raw, context);
      return false;
    }
    const input = context === "box" ? $("boxSku") : $("receiveSku");
  ...
  async function receive() {
    ...
    if (!await requireKnownProduct(product, raw, "receive")) return;
  ...
  async function addBox() {
    ...
    const raw = $("boxSku").value;
    const product = await productByScan(raw);
    if (!product) return msg("กรุณายิงสินค้า", "error");
    if (!await requireKnownProduct(product, raw, "box")) return;
  ...
  async function addReceivedToBox() {
    if (S.box.status !== "DRAFT") return msg("กล่องปิดแล้ว ต้องเปิดกล่องก่อนแก้ไข", "error");
    const blocked = S.receipts.filter((row) => Number(row.good) > 0 && !row.product_id);
    if (blocked.length) return msg(`พบ ${blocked.length} รายการที่ไม่มี SKU จริง กรุณาลบและสร้างสินค้าให้เรียบร้อยก่อน`, "error");
    const rows = S.receipts.filter((row) => Number(row.good) > 0 && row.product_id);
  ...
  async function scan() {
    ...
    if (/^TKN-B-/i.test(raw)) {
      ...
      const product = await productByScan(raw);
      if (!product?.id) {
        await requireKnownProduct(product, raw, "receive");
        $("scanResult").textContent = "ไม่รับรหัสที่ไม่พบในหน้าจัดการสินค้า";
        return;
      }
  ...
  function updateReceiveTotal() {
    const validRows = (Array.isArray(S.receipts) ? S.receipts : [])
      .filter((row) => Number(row?.actual || 0) > 0 && row?.product_id);
    const totalItems = validRows.length;
    const totalPieces = validRows.reduce((sum, row) => sum + Math.max(0, Number(row.actual || 0)), 0);
    const output = $("receiveTotalPieces");
    if (!output) return;
    output.textContent = `${integer(totalItems)} รายการ · ${integer(totalPieces)} ชิ้น`;
    output.dataset.totalItems = String(totalItems);
    output.dataset.totalPieces = String(totalPieces);
  }
  ...
  function applyUrlIntent() {
    ...
    if (tab === "receive") {
      ...
      $("receiveSku").focus();
      msg(created ? "สร้างสินค้าแล้ว ระบบใส่รหัสเดิมกลับมาให้ กรุณาตรวจจำนวนและยืนยัน" : "รับรหัสจากโหมดมือถือแล้ว ตรวจจำนวนและกดยืนยัน", "success");
    } else if (tab === "box") {
      $("boxSku").value = scanValue.replace(/^TKN-P-/i, "");
      $("boxSku").focus();
      msg(created ? "สร้างสินค้าแล้ว ระบบใส่รหัสเดิมกลับมาให้ พร้อมเพิ่มเข้ากล่อง" : "รับรหัสจากโหมดมือถือแล้ว ใส่จำนวนและกดเพิ่มเข้ากล่อง", "success");
    } else {
      activateTab("check");

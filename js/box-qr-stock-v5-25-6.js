(() => {
  ...
  function newBox() {
    S.box = { id: code("TKN-B"), draftId: null, shortCode: "", category: "", categoryCode: "", zoneCode: "A", status: "DRAFT", location: "", created_at: now(), cloudId: null };
    S.boxItems = [];
  ...
  async function closeBox() {
    ...
    try {
      ...
      if (error) throw error;
      msg("ปิดกล่องและสร้าง QR แล้ว จำนวนยืนยันจากสินค้าที่สแกนเข้ากล่องจริง", "success");
    } catch (error) {
      console.warn(error);
  ...
  async function loadCloudStats() {
    try {
      ...
      S.cloudStats = { sessions: sessions.count || 0, drafts: drafts.count || 0, closed: closed.count || 0 };
      renderStats();
    } catch (error) {
  ...
  function renderFinalSummary() {
    ...
    $("finalBoxSummary").innerHTML = `
      ...
      <article><span>สถานะ</span><b>${esc(S.box.status)}</b></article>`;
  ...
  function applyBoxPrintSettings(persist = true, redraw = true) {
    ...
    const qrSize = Math.max(22, Math.min(profile.width - 8, profile.height * (settings.showDetails ? 0.62 : 0.78)));
  ...
  function bindEvents() {
    ...
    bind("newRoundBtn", "click", () => {
      if (S.boxItems.length && !confirm("เริ่มรอบและกล่องใหม่? รายการในกล่องปัจจุบันจะถูกนำออกจากหน้าจอนี้")) return;
      newSession();
      newBox();
      S.receipts = [];

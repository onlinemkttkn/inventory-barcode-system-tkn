(() => {
  ...
  async function closeBox() {
    ...
    try {
      ...
      const branchId = currentBranchId();
      if (!branchId) throw new Error("กรุณาเลือกสาขาก่อนปิดกล่อง");
      const { data: waiting, error: waitingError } = await supabaseClient.rpc("tkn_v5283_close_box_to_waiting", {
        p_box_code: S.box.id,
      ...
      save(false);
      msg("ปิดกล่องแล้ว · บันทึกประวัติและย้ายไปรอเข้าสต็อกแล้ว (ยังไม่เพิ่มยอดสต็อก)", "success");
      void loadBoxHistory();
    } catch (error) {
      console.warn(error);

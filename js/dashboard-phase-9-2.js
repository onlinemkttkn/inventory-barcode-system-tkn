...
// Isolated configuration for Dashboard Phase 9.2.
// This file does not import or overwrite js/supabase-config.js or js/supabase-client.js.
const DASHBOARD_SUPABASE_URL = "https://wkozeuxyhqcmiatssviq.supabase.co";
...
const supabaseClient = window.supabase.createClient(
  ...
  DASHBOARD_SUPABASE_PUBLISHABLE_KEY,
  {
    auth: {
      ...
      storageKey: "tkn-dashboard-phase-9-2-auth"
    }
  }
);
...
// Warn only inside the upgraded dashboard when required Phase 9.2 pages are missing.
window.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll("a.phase-card").forEach((link) => {
    link.addEventListener("click", () => {
      sessionStorage.setItem("tkn_phase_9_2_source", "dashboard-phase-9-2");
    });
  });
});

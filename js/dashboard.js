...
function dashboardBranchLabel(code) {
  ...
  return ({ BR001: 'โกดังเก็บสินค้า', BR002: 'สาขา 1', ONLINE: 'ONLINE' })[normalized] || String(code || '-');
}
...
E.loginForm?.addEventListener('submit', async (event) => {
  ...
  if (!email || !password) return msg(E.loginMessage, 'กรุณากรอกอีเมลและรหัสผ่าน', 'error');
  try {
    if (submitButton) submitButton.disabled = true;
    msg(E.loginMessage, 'กำลังเข้าสู่ระบบ...');
    const { error } = await supabaseClient.auth.signInWithPassword({ email, password });
    if (error) return msg(E.loginMessage, error.message, 'error');
    E.password.value = '';
    window.location.replace('./dashboard.html?view=latest&v=5.13.1');
  } catch (error) {

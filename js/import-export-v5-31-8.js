...
function makeThaiImportName(productName, row = {}) {
  const made = window.TKNThaiProductName?.make({
    originalName: productName,
    mainCategory: row.category_name || row.category_code,
    subCategory: row.sub_category,
    sku: row.product_code
  });
  return made || { name: productName, originalName: productName, changed: false, status: 'SOURCE_FALLBACK' };
}

function descriptionWithSourceName(description, sourceName, thaiName) {
  return window.TKNThaiProductName?.sourceNote(description || '', sourceName || '', thaiName || '', 'ชื่อเดิมต้นทาง') || description || null;
}
...
async function validateRows() {
  ...
  const [
    ...
    supabaseClient.from("categories").select("code,name"),
  ...
  checkedRows = rawRows.map((row) => {
    ...
    const sourceProductName = String(row.product_name || "").trim();
    const categoryCode = String(row.category_code || "")
      .trim()
      .toUpperCase();
    const categoryName = categoryNameByCode.get(categoryCode) || String(row.category_name || "").trim();
    const thaiProductName = makeThaiImportName(sourceProductName, {
      ...row,
      product_code: productCode,
      category_name: categoryName,
    });
    const productName = thaiProductName.name;
    ...
    return {
      ...
      product_name_th_status: thaiProductName.status,
  ...
  for (let index = 0; index < validRows.length; index += 1) {
    ...
    const { data, error } = await supabaseClient.rpc(
      "import_product_row",
      {
        ...
        p_description: descriptionWithSourceName(row.description, row.source_product_name, row.product_name),
      }
    );

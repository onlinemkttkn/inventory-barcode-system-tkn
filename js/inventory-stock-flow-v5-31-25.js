(() => {
  ...
  async function loadIntake() {
    try {
      ...
      if ($('hubIntakeRows')) {
        $('hubIntakeRows').innerHTML = rows.map(x => `<tr><td>${fmt(x.closed_at)}</td><td><b>${esc(x.box_code)}</b></td><td>${esc(x.sku_count)} SKU · ${esc(x.total_quantity)} ชิ้น</td><td>${esc(x.branch_name||'สำนักงานใหญ่ / คลังหลัก')}</td><td><a href="./stock-intake.html?scan=${encodeURIComponent(x.box_code)}">ตรวจรับ</a></td></tr>`).join('') || '<tr><td colspan="5">ไม่มีคิว</td></tr>';
      }
    } catch (e) {

(() => {
  ...
  const REQUIRED_SOURCE_GROUPS = [
    { label: "Tracking หรือ Order/Return Number", anyOf: ["tracking_number", "handover_tracking_number", "order_number", "return_number", "platform_return_item_id", "rms_return_item_id"] },
    { label: "sku_id / seller_sku", anyOf: ["sku_id"] },
    { label: "item_name / product_name", anyOf: ["item_name"] },
  ];
  const SOURCE_HEADER_ALIASES = {
    tracking_number: ["tracking_number", "tracking number", "tracking no", "tracking no.", "tracking code", "package_number", "package number", "package no", "package no.", "shipping tracking number", "airway bill", "awb", "inbound tracking number", "เลขพัสดุ", "หมายเลขพัสดุ"],
    handover_tracking_number: ["handover tracking number", "origin tms package uuid", "lel handover package uuid"],
    order_number: ["order_number", "order number", "order no", "order no.", "order id", "order_id", "customer order number", "เลขคำสั่งซื้อ", "หมายเลขคำสั่งซื้อ"],
    return_number: ["return number"],
    platform_return_item_id: ["platform return item id"],
    rms_return_item_id: ["rms return item id"],
    ...
    main_category: ["main_category", "main category", "category", "category name", "item category name", "หมวดหลัก", "หมวดหมู่หลัก", "หมวดหมู่"],
    sub_category: ["sub_category", "sub category", "subcategory", "sub-category", "product category", "platform category id", "หมวดย่อย", "หมวดหมู่ย่อย"],
    return_status: ["return_status", "return status", "reverse status", "refund status", "สถานะคืนสินค้า", "สถานะการคืน"],
    lzd_return_type: ["lzd return type"],
    qc_result: ["qc result"],
    closure_outcome: ["logistic closure outcome"],
    closure_type: ["logistic closure type"],
    handover_type: ["handover type"],
    order_status: ["order_status", "order status", "status", "สถานะคำสั่งซื้อ", "สถานะ"],
    item_price_pp: ["item_price_pp", "item price", "paid price", "unit price", "product unit price", "ราคา", "ราคาสินค้า"],
  ...
  function sourceStatusText(row) {
    return Array.from(new Set([row?.return_status, row?.order_status].map(normalizeText).filter(Boolean))).join(" · ");
  }
  ...
  async function readCsvText(file) {
    ...
    const decode = (name) => new TextDecoder(name, { fatal: false }).decode(buffer).replace(/^\uFEFF/, "");
    let text = decode(encoding);

    if (encoding === "utf-8") {
      const badUtf8 = (text.match(/\uFFFD/g) || []).length;
      if (badUtf8 > Math.max(5, text.length * 0.001)) {
        try {
          const thaiText = decode("windows-874");
          const badThai = (thaiText.match(/\uFFFD/g) || []).length;
          if (badThai < badUtf8) text = thaiText;
        } catch (_) {
          // Browser บางรุ่นไม่มี windows-874 ให้ใช้ UTF-8 ต่อไป
        }
      }
    }
    return text;
  ...
  async function processSourceFile(file) {
    ...
    try {
      if (file.size > 50 * 1024 * 1024) throw new Error("ไฟล์มีขนาดเกิน 50 MB กรุณาแบ่งไฟล์ก่อนนำเข้า");
      ...
      const headers = sourceHeaders.map(canonicalSourceHeader);
      const missing = REQUIRED_SOURCE_GROUPS.filter((group) => !group.anyOf.some((header) => headers.includes(header)));
      if (missing.length) {
        const found = sourceHeaders.filter(Boolean).slice(0, 14).join(", ");
        throw new Error(`ไฟล์ขาดคอลัมน์ที่จำเป็น: ${missing.map((group) => group.label).join(", ")} | คอลัมน์ที่พบ: ${found || "ไม่พบหัวตาราง"}`);
      }
      const isReturnCenterFile = sourceHeaders.some((header) => normalizeAliasHeader(header) === "inbound tracking number") || sourceHeaders.some((header) => normalizeAliasHeader(header) === "platform return item id");
      const positions = headers.reduce((result, header, index) => {
        if (!result[header]) result[header] = [];
        result[header].push(index);
        return result;
      }, {});
      const sourceValue = (values, canonical) => {
        for (const index of positions[canonical] || []) {
          const value = normalizeText(values[index]);
          if (value) return value;
        }
        return "";
      };
      const sourceValueAny = (values, canonicals) => {
        for (const canonical of canonicals) {
          const value = sourceValue(values, canonical);
          if (value) return value;
        }
        return "";
      };
      ...
      for (let index = 1; index < matrix.length; index += 1) {
        if (index % 500 === 0) {
          const percent = 38 + Math.min(32, Math.round((index / Math.max(1, matrix.length - 1)) * 32));
          setParseProgress(percent, `กำลังตรวจรายการ ${integer(index)} / ${integer(matrix.length - 1)}...`);
          await nextFrame();
        }
        const values = matrix[index];
        const orderNumber = sourceValueAny(values, ["order_number", "return_number", "platform_return_item_id", "rms_return_item_id"]);
        const trackingNumber = sourceValueAny(values, ["tracking_number", "handover_tracking_number"]) || orderNumber;
        const skuId = sourceValue(values, "sku_id");
        const itemName = sourceValue(values, "item_name");
        if (!trackingNumber || !skuId || !itemName) { removed += 1; continue; }
        const key = recordKey(trackingNumber, skuId);
        const quantity = normalizeQuantity(sourceValue(values, "item_quantity")) || 1;
        ...
        aggregated.set(key, {
          ...
          item_name: itemName,
          main_category: sourceValue(values, "main_category") || "Lazada",
          sub_category: sourceValue(values, "sub_category") || "ยังไม่จัดหมวด",
          return_status: ["return_status", "lzd_return_type", "qc_result", "closure_outcome", "closure_type", "handover_type"].map((field) => sourceValue(values, field)).filter(Boolean).join(" · "),
          order_status: sourceValue(values, "order_status"),
          source_item_price: sourceValue(values, "item_price_pp"),
          product_code: sourceValue(values, "product_code"),
          cost_price: sourceValue(values, "cost_price"),
          selling_price: sourceValue(values, "selling_price"),
          return_number: sourceValue(values, "return_number"),
          platform_return_item_id: sourceValue(values, "platform_return_item_id"),
          rms_return_item_id: sourceValue(values, "rms_return_item_id"),
          merged_rows: 1,
      ...
      refreshAll();
      const formatLabel = isReturnCenterFile ? "Lazada Return Center" : "Lazada Order Export";
      setMessage(`นำเข้า ${formatLabel} สำเร็จ ${integer(rows.length)} รายการ จาก ${integer(state.skuRows.length)} SKU สามารถเลือกหมวดและทยอยกรอกราคาได้ทันที`, "success");
      ...
      for (let index = 0; index < jobs.length; index += 1) {
        ...
        try {
          if (match.type === "existing") {
            ...
            const sourceName = item.item_name || `Lazada SKU ${skuId}`;
            const defaultCategoryName = String(E.defaultCategory.selectedOptions?.[0]?.textContent || "")
              .replace(/^[^—]+—\s*/, "")
              .trim();
            const thaiName = window.TKNThaiProductName?.make({
              originalName: sourceName,
              mainCategory: item.main_category || defaultCategoryName,
              subCategory: item.sub_category,
              sku: item.product_code || skuId
            }) || { name: `สินค้าทั่วไป รหัส ${item.product_code || skuId}` };
            result = await supabaseClient.rpc("import_product_row", {
              p_product_code: normalizeText(item.product_code || skuId),
              p_name: thaiName.name,
              p_barcode: null,
              ...
              p_description: window.TKNThaiProductName?.sourceNote(`LAZADA_SKU:${skuId}`, sourceName, thaiName.name, 'ชื่อเดิม Lazada') || `LAZADA_SKU:${skuId}`,
            });
          }

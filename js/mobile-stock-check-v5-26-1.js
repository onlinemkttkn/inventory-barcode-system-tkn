(() => {
  ...
  const RECENT_KEY = "tkn_mobile_stock_recent_v5261";
  const COUNT_KEY = "tkn_mobile_stock_count_drafts_v514";
  const BRANCH_KEY = "tkn_working_branch_v5261";
  const state = {
  ...
  async function loadBranches() {
    ...
    if (saved && state.branches.some((b) => b.id === saved)) $("workingBranch").value = saved;
    else if (state.branches.length === 1) $("workingBranch").value = state.branches[0].id;
  }
  ...
  async function startCamera() {
    ...
    try {
      ...
      state.controls = await state.reader.decodeFromConstraints({ video:{ deviceId:preferred?.deviceId ? { exact:preferred.deviceId } : undefined, facingMode:{ ideal:"environment" }, width:{ ideal:1280 }, height:{ ideal:720 } }, audio:false }, $("cameraVideo"), async (result) => {
        ...
        if (!code || (code === state.lastCode && now - state.lastAt < 1800)) return;
        state.lastCode = code; state.lastAt = now; await handleCode(code, true);
      });
  ...
  async function handleCode(raw, fromCamera = false) {
    const code = normalizeCode(raw); if (!code || state.busy) return;
  ...
  async function lookupBox(boxCode) {
    const { data, error } = await supabaseClient.rpc("tkn_v5261_get_box_context", { p_box_code:boxCode });
    if (error) {
      if (/function .* does not exist|schema cache/i.test(error.message || "")) throw new Error("ยังไม่ได้ติดตั้ง SQL v5.26.1");
      throw error;
    }
  ...
  function renderBox(d) {
    ...
    $("resultPanel").innerHTML = `
      ...
      <div class="box-item-list"><b>สินค้าภายในกล่อง</b>${items.length ? items.map((i)=>`<div class="box-item"><span><b>${esc(i.product_name || i.sku)}</b><small>${esc(i.sku)} · ทุน ${money(i.unit_cost)} · ขาย ${money(i.unit_price)}</small></span><b>${num(i.quantity)}</b></div>`).join("") : '<div class="box-item"><span>กล่องว่าง</span><b>0</b></div>'}</div>
      <div class="box-flow-actions">
        ${state.mode === "ISSUE_BOX" ? '<button class="btn primary" data-action="BOX_ACTION" type="button">เลือกการเบิก / รับกล่อง</button>' : ""}
        ${can("pos.box_sale.create") && b.location_state === "WAREHOUSE" && String(b.status).toUpperCase() === "CLOSED" ? `<a class="btn ghost" href="./pos-box-sale.html?scan=${encodeURIComponent(b.box_code)}&branch=${encodeURIComponent(b.branch_id || selectedBranchId())}">ไป POS ขายยกกล่อง</a>` : ""}
        <button class="btn ghost" type="button" data-action="COUNT">ตรวจนับกล่อง</button>
      </div>`;
    if (state.mode === "ISSUE_BOX") setMessage("ตรวจข้อมูลกล่องแล้ว กดเลือกการเบิก หรือสแกนรับกล่องระหว่างทาง", "success");
  }
  ...
  function openBoxActionDialog() {
    ...
    if (b.location_state === "IN_TRANSIT") {
      ...
      if (can("inventory.issue") || can("product.manage")) buttons.push('<button class="flow-action primary-action" type="button" data-box-action="ISSUE"><b>เบิกขายหน้าร้าน</b><small>นำสินค้าทั้งหมดออกจากกล่อง โดยยังไม่ลดสต็อกสาขา</small></button>');
      ...
      if (can("pos.box_sale.create")) buttons.push(`<a class="flow-action" href="./pos-box-sale.html?scan=${encodeURIComponent(b.box_code)}&branch=${encodeURIComponent(selectedBranchId())}"><b>ขายยกกล่องใน POS</b><small>ขายตาม SKU หรือราคาเหมา รองรับสแกนหลายกล่อง</small></a>`);
    }
  ...
  async function issueCurrentBox() {
    ...
    if (d.box.branch_id && d.box.branch_id !== selectedBranchId()) return setMessage("กล่องอยู่คนละสาขากับสาขาที่เลือก", "error");
    if (!confirm(`ยืนยันเบิกสินค้าทั้งหมดในกล่อง ${d.box.box_code}\n${num(d.total_sku)} SKU รวม ${num(d.total_quantity)} ชิ้น\n\nสินค้าไปอยู่หน้าร้าน ส่วนกล่องกลับเป็นกล่องว่าง`)) return;
    $("boxActionDialog").close(); state.busy = true; setMessage("กำลังเบิกสินค้าทั้งกล่อง...", "info");
    try {
      const { data, error } = await supabaseClient.rpc("tkn_v5261_issue_box_to_storefront", { p_box_code:d.box.box_code, p_branch_id:selectedBranchId() });
      if (error) throw error; const r = normalizeRpc(data);
      setMessage(`เบิกกล่อง ${r.box_code} สำเร็จ · ${num(r.total_sku)} SKU รวม ${num(r.total_quantity)} ชิ้น พร้อมขายหน้าร้าน`, "success");
      addRecent({ type:"เบิกหน้าร้าน", code:r.box_code, name:`เบิกกล่อง ${r.box_code}` }); await lookupBox(r.box_code); feedback(true);
    } catch (e) { feedback(false); setMessage(e.message || "เบิกกล่องไม่สำเร็จ", "error"); }
    finally { state.busy = false; }
  }
  ...
  function setMode(mode) {
    ...
    $("cameraHelp").textContent = mode === "LOOKUP" ? "สแกนสินค้า Barcode หรือ QR กล่อง เพื่อดูจำนวนและตำแหน่ง" : "รับเฉพาะ QR กล่อง เพื่อเบิกหน้าร้าน ส่งไปสาขา หรือรับกล่องจากสาขา";
    ...
    setMessage(mode === "LOOKUP" ? "โหมดดูสต็อก ไม่เปลี่ยนข้อมูล" : "โหมดเบิกกล่อง: ทุกการทำงานใช้สินค้าทั้งกล่อง", "info");
  }

  function bindEvents() {
    $("backBtn").onclick = () => { stopCamera(); window.TKNSafeBack?.go?.({ fallback:"./dashboard.html" }) || (location.href="./dashboard.html"); };
    $("workingBranch").onchange = () => { localStorage.setItem(BRANCH_KEY, selectedBranchId()); sessionStorage.setItem("tkn_inventory_branch_id", selectedBranchId()); setMessage(selectedBranchId() ? `เลือกสาขา ${selectedBranch()?.name}` : "กรุณาเลือกสาขา", selectedBranchId()?"success":"info"); };
    ...
    $("boxActionButtons").onclick=(e)=>{const a=e.target.closest("[data-box-action]")?.dataset.boxAction;if(a==="ISSUE")issueCurrentBox();if(a==="TRANSFER")openTransferDialog();if(a==="RECEIVE")receiveCurrentBox()};
    $("transferForm").onsubmit=transferCurrentBox; $("transferClose").onclick=$("transferCancel").onclick=()=>$("transferDialog").close();
    $("receiveNextDialog").onclick=(e)=>{const n=e.target.closest("[data-next]")?.dataset.next;if(n==="ISSUE"){e.preventDefault();$("receiveNextDialog").close();issueCurrentBox()}if(n==="FORWARD"){e.preventDefault();$("receiveNextDialog").close();openTransferDialog()}};
  ...
  async function initialize() {
    try {
      ...
      state.access=access;
      if (!["owner","admin"].includes(role()) && !["inventory.view","inventory.issue","inventory.transfer","inventory.receive","inventory.box_transfer","product.manage","pos.box_sale.create"].some((p)=>permissions().has(p))) { location.replace(access.landing_page||"./dashboard.html"); return; }

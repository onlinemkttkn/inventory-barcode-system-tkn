(() => {
  ...
  function renderBox(d) {
    ...
    if (state.mode === "ISSUE_BOX") setMessage("ตรวจข้อมูลกล่องแล้ว สามารถยืนยันเบิกสินค้าทั้งกล่องเข้า POS ได้ทันที", "success");
  }
  ...
  function openBoxActionDialog() {
    ...
    if (b.location_state === "IN_TRANSIT") {
      ...
      if (can("inventory.issue") || can("product.manage")) buttons.push('<button class="flow-action primary-action" type="button" data-box-action="ISSUE_WHOLE"><b>เบิกทั้งกล่องเข้า POS</b><small>ยืนยัน QR กล่องครั้งเดียว ระบบจะนำสินค้าทั้งหมดในกล่องออกเป็นสินค้าพร้อมขายหน้าร้าน</small></button>');
  ...
  async function issueWholeBoxToStorefront() {
    ...
    const b = d.box;
    if (b.branch_id && b.branch_id !== selectedBranchId()) return setMessage("กล่องอยู่คนละสาขากับสาขาที่เลือก", "error");
    if (String(b.status || "").toUpperCase() !== "CLOSED" || !(d.items || []).length) return setMessage("กล่องต้องปิดและมีสินค้าก่อนเบิก", "error");
    if (String(d.classification?.workflow_status || "").toUpperCase() !== "IN_STOCK") return setMessage("กล่องนี้ยังไม่อยู่สถานะ IN_STOCK กรุณาตรวจรับเข้าสต็อกก่อน", "error");
    ...
    try {
      ...
      setMessage(e.message || "เบิกทั้งกล่องเข้า POS ไม่สำเร็จ", "error");
    } finally {
      state.busy = false;
    }
  }

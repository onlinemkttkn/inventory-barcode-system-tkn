...
els.returnButton.addEventListener('click', () => {
  ...
  const url =
    `./sales-return-v2-1.html?sale_id=${encodeURIComponent(state.selectedBill.id)}`
    + `&sale_no=${encodeURIComponent(saleNo)}`;

  const width = Math.min(1120, Math.max(760, window.screen.availWidth - 120));
  const height = Math.min(900, Math.max(680, window.screen.availHeight - 100));
...
window.addEventListener('message', (event) => {
  ...
  if (event.data?.type !== 'TKN_SALE_RETURN_SUCCESS') return;

  if (els.billDialog?.open) {
    els.billDialog.close();
  }

  searchBills();

  const returnNo = event.data.returnNo || '';
  els.resultMessage.textContent =
    `คืนสินค้าสำเร็จ${returnNo ? ` · ${returnNo}` : ''}`;
});

(() => {
  ...
  const E = {
    markupSelect: root.querySelector('#bulkMarkupSelect'),
    markupCustom: root.querySelector('#bulkMarkupCustomPercent'),
    markupCustomLabel: root.querySelector('#bulkMarkupCustomLabel'),
  ...
  function syncCustomMarkup() {
    const isCustom = E.markupSelect.value === 'CUSTOM';
    E.markupCustomLabel.hidden = !isCustom;
    E.markupCustom.disabled = !isCustom;
    if (isCustom) requestAnimationFrame(() => E.markupCustom.focus());
  }

  function markupPercent() {
    return Number(E.markupSelect.value === 'CUSTOM' ? E.markupCustom.value : E.markupSelect.value);
  }
  ...
  E.markupSelect.addEventListener('change', syncCustomMarkup);
  E.markupCustom.addEventListener('input', () => {
    if (E.markupSelect.value !== 'CUSTOM') E.markupSelect.value = 'CUSTOM';
    syncCustomMarkup();
  });
  syncCustomMarkup();

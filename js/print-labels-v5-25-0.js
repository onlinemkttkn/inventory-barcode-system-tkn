...
const SETTINGS_KEY = 'tkn_print_label_settings_v5250';
...
function loadSettings() {
  ...
  if (!el.barcodeSource.value) el.barcodeSource.value = 'product_code';
}
...
function getBarcodeValue(product) {
  const sku = PATTERN?.barcodeValue(product?.product_code) || String(product?.product_code || '').trim();
  if (el.barcodeSource.value === 'barcode' && (product?.source_barcode || product?.barcode)) return String(product.source_barcode || product.barcode).trim();
  return sku;
}

function getProductQrValue(product) {
  return PATTERN?.qrValue(product?.product_code) || (product?.product_code ? `TKN-P-${product.product_code}` : '');
}
...
function presetConfig() {
  ...
  return {
    ...base,
    pageWidth: el.paperPreset.value === 'custom' ? numberSetting(el.customWidth, 50, 20, 210) : base.pageWidth,
    pageHeight: el.paperPreset.value === 'custom' ? numberSetting(el.customHeight, 30, 15, 297) : base.pageHeight,
    qrOnly: numberSetting(el.qrSize, base.qrOnly, 28, 220),
    qrBoth: Math.max(28, Math.round(numberSetting(el.qrSize, base.qrBoth, 28, 220) * 0.82)),
    barcodeHeight: numberSetting(el.barcodeHeight, base.barcodeHeight, 12, 100),
    barcodeBothHeight: Math.max(12, Math.round(numberSetting(el.barcodeHeight, base.barcodeBothHeight, 12, 100) * 0.82)),
    fontSize: numberSetting(el.labelFontSize, base.fontSize, 6, 30)
  };
}

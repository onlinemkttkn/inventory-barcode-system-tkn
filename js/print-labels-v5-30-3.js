...
function saveSettings() {
  ...
  window.TKNPrintPlatform?.saveProductSettings?.({
    ...
    columns:data.labelColumns,gap:data.labelGap,pageMargin:data.pageMargin,codeMode:data.codeMode,conciseName:data.conciseName,
...
function updatePrintPageStyle() {
  ...
  style.textContent=resolvedMode==='SHEET'||preset.startsWith('a4-')?`@page{size:A4 portrait;margin:${margin}mm}`:`@page{size:${cfg.labelWidth}mm ${cfg.labelHeight}mm;margin:${margin}mm}`;
}

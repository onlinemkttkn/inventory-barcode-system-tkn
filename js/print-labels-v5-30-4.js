const el = {
  ...
  printerDpi: document.getElementById("printerDpi"),
  ...
  rowPresetHint: document.getElementById("rowPresetHint"),
...
function saveSettings() {
  const data = {
    ...
    printerMode: el.printerMode?.value || "AUTO",
    dpi: Number(el.printerDpi?.value || 300),
    ...
    columnsMode: el.labelColumns.dataset.userEdited==='1' ? 'manual' : 'recommended',
  ...
  window.TKNPrintPlatform?.saveProductSettings?.({
    preset:data.paperPreset,printerMode:data.printerMode,dpi:data.dpi,customWidth:data.customWidth,customHeight:data.customHeight,
    columns:data.labelColumns,columnsMode:data.columnsMode,gap:data.labelGap,pageMargin:data.pageMargin,codeMode:data.codeMode,conciseName:data.conciseName,
    showName:data.showName,showPrice:data.showPrice,showProductCode:data.showProductCode,showBarcodeText:data.showBarcodeText
  });
}
...
function loadSettings() {
  try {
    ...
    const shared=window.TKNPrintPlatform?.getProductSettings?.()||{};
...
const PRESET_CONFIG = Object.freeze({
  '20x10': {pageWidth:20,pageHeight:10,qrOnly:18,qrBoth:14,barcodeHeight:12,barcodeBothHeight:8,barcodeWidth:.5,fontSize:5},
  '30x20': {pageWidth:30,pageHeight:20,qrOnly:28,qrBoth:22,barcodeHeight:18,barcodeBothHeight:13,barcodeWidth:.65,fontSize:6},
  '40x20': {pageWidth:40,pageHeight:20,qrOnly:28,qrBoth:22,barcodeHeight:18,barcodeBothHeight:13,barcodeWidth:.75,fontSize:6},
  '40x70': {pageWidth:40,pageHeight:70,qrOnly:76,qrBoth:62,barcodeHeight:46,barcodeBothHeight:34,barcodeWidth:1.0,fontSize:9},
  '50x20': {pageWidth:50,pageHeight:20,qrOnly:28,qrBoth:22,barcodeHeight:18,barcodeBothHeight:13,barcodeWidth:.9,fontSize:6},
  '50x30': {pageWidth:50,pageHeight:30,qrOnly:48,qrBoth:38,barcodeHeight:27,barcodeBothHeight:22,barcodeWidth:1.0,fontSize:8},
  '50x40': {pageWidth:50,pageHeight:40,qrOnly:68,qrBoth:56,barcodeHeight:38,barcodeBothHeight:30,barcodeWidth:1.1,fontSize:9},
  '40x30': {pageWidth:40,pageHeight:30,qrOnly:42,qrBoth:32,barcodeHeight:24,barcodeBothHeight:19,barcodeWidth:.85,fontSize:7},
  '58x40': {pageWidth:58,pageHeight:40,qrOnly:70,qrBoth:58,barcodeHeight:40,barcodeBothHeight:31,barcodeWidth:1.2,fontSize:9},
...
function presetConfig() {
  ...
  const customHeight = numberSetting(el.customHeight, 30, 10, 297);
...
function applyRecommendedColumnsForPreset({force=false}={}) {
  const preset=el.paperPreset.value;
  const recommended=window.TKNLabelLayout?.recommendedColumns?.(preset, null);
  if(!recommended) {
    if(el.rowPresetHint) el.rowPresetHint.textContent='Preset เดิมยังใช้ค่าจำนวนต่อแถวที่ตั้งไว้ ไม่เปลี่ยนอัตโนมัติ';
    return;
  }
  if(force || el.labelColumns.dataset.userEdited!=='1') {
    el.labelColumns.value=String(recommended);
  }
  const cfg=window.TKNLabelLayout?.productProfile?.({preset,customWidth:Number(el.customWidth.value||50),customHeight:Number(el.customHeight.value||30),dpi:Number(el.printerDpi?.value||300)});
  const compact=cfg?.density==='micro'?' · โหมด Micro จะซ่อนชื่อและราคาอัตโนมัติเพื่อกันข้อมูลทับกัน':'';
  if(el.rowPresetHint) el.rowPresetHint.textContent=`${preset.replace('x',' × ')} มม. แนะนำ ${recommended} ต่อแถว${compact} — สามารถแก้จำนวนต่อแถวเองได้`;
}
...
function updatePrintPageStyle() {
  ...
  const printerMode=el.printerMode?.value||'AUTO';
  const resolvedMode=window.TKNPrintPlatform?.resolvePrintMode?.({printerMode,columns,pageMargin:margin},cfg.labelWidth)||(columns===1?'ROLL':'SHEET');
  const rollWidth=window.TKNPrintPlatform?.rowPhysicalWidth?.({columns,gap,pageMargin:margin},cfg.labelWidth) || (columns*cfg.labelWidth+Math.max(0,columns-1)*gap+2*margin);
  style.textContent=resolvedMode==='SHEET'||preset.startsWith('a4-')?`@page{size:A4 portrait;margin:${margin}mm}`:`@page{size:${rollWidth}mm ${cfg.labelHeight}mm;margin:${margin}mm}`;
}
...
el.paperPreset.addEventListener('change',()=>{
  el.labelColumns.dataset.userEdited='';
  applyRecommendedColumnsForPreset({force:true});
  syncCodeModeUi(); saveSettings(); if(queue.size) renderLabels();
});
el.labelColumns.addEventListener('change',()=>{
  el.labelColumns.dataset.userEdited='1';
  syncCodeModeUi(); saveSettings(); if(queue.size) renderLabels();
});
[
...
;['printerMode','printerDpi'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>{saveSettings();renderLabels();}));

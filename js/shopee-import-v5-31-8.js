      ...
      for (let index = 0; index < jobs.length; index += 1) {
        ...
        try {
          if (match.type === "existing") {
            ...
            const sourceName = item.item_name || `Shopee SKU ${skuId}`;
            ...
            result = await supabaseClient.rpc("import_product_row", {
              ...
              p_description: window.TKNThaiProductName?.sourceNote(`SHOPEE_SKU:${skuId}`, sourceName, thaiName.name, 'ชื่อเดิม Shopee') || `SHOPEE_SKU:${skuId}`,
            });
          }

(() => {
  ...
  const LABEL_SETTINGS_VERSION = 11;
  ...
  function renderBox() {
    ...
    $('labelPreview').innerHTML = units.map(({ item, index, quantity }) => `<article class="sp-label tkn-label-product" data-label="${esc(item.id)}" data-unit="${index}">
      <div class="sp-label-qr-status" hidden></div><canvas class="sp-label-qr tkn-label-qr"></canvas>
      <svg class="sp-label-barcode tkn-label-barcode" aria-label="Barcode ${esc(item.sku)}"></svg>
      <small class="tkn-label-sku">${esc(ensureLotSku(item))}${quantity > 1 ? ` · ${index}/${quantity}` : ''}</small>
      <h4 class="tkn-label-name">${esc(item.name)}</h4>
    </article>`).join('');
  ...
  async function drawLabels() {
    ...
    await Promise.all(elements.map(async (element) => {
      ...
      try {
        ...
        ensureLotSku(item);
        await toCanvas(canvas, PATTERN?.qrValue?.(item.sku) || `TKN-P-${item.sku}`, { width: pixelSize, margin: 1, errorCorrectionLevel: 'M' });
        if (typeof window.JsBarcode !== 'function') throw new Error('ระบบ Barcode ยังไม่พร้อม');
        const barcodeValue = PATTERN?.barcodeValue?.(item.sku) || String(item.sku || '').trim();
        ...
        window.JsBarcode(barcodeSvg, barcodeValue, {
          ...
          width: barWidth, height: Math.max(18, Number(state.labelSettings?.barcodePx || window.TKNLabelLayout?.mmToPx?.(state.labelSettings?.barcodeHeight || 6, state.labelSettings?.dpi || 300) || 34)), background: '#fff', lineColor: '#000',
        });
  ...
  async function finalizeBoxIdentity(client) {
    ...
    if (categories.length > 1) {
      throw new Error(`กล่องนี้มีหลายประเภท (${categories.map((row) => row.label).join(', ')}) กรุณาแยกเป็นคนละกล่องก่อนปิด`);
    }
    let category = categories[0] || BOX_CODE.resolveCategory(box.category || '');
  ...
  function renderClose() {
    ...
    const categoryText = categories.map((row) => row.label).join(', ') || state.box.category || '-';
  ...
  async function closeBox() {
    ...
    if (!confirm(`ปิดผนึกกล่อง ${identity.short} · ${identity.categoryLabel} · โซน ${identity.zone} · ${quantity} ชิ้น?`)) return;
    ...
    if (client) {
      try {
        ...
        const waitingResult = await client.rpc('tkn_v5300_close_box_to_stock_intake', {
          p_box_code: box.code,
          p_category_text: box.category || identity.categoryLabel || null,

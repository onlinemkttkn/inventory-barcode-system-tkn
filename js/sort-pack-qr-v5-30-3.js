(() => {
  ...
  const LABEL_SETTINGS_VERSION = 12;
  ...
  function applyLabelSettings(save = true) {
    ...
    printStyle.textContent=printMode==='ROLL'?`@page{size:${width}mm ${height}mm;margin:${Number(settings.pageMargin||0)}mm}`:`@page{size:A4 portrait;margin:${Number(settings.pageMargin||0)}mm}`;
  ...
  function fillLabelSettingInputs(){const shared=window.TKNPrintPlatform?.getProductSettings?.()||{},settings={...DEFAULT_LABEL_SETTINGS,...(state.labelSettings||{}),...shared};state.labelSettings=settings;if(!$('labelSizePreset'))return;$('labelPrinterMode').value=settings.printerMode||'AUTO';$('labelPrinterDpi').value=String(settings.dpi||300);$('labelSizePreset').value=[...$('labelSizePreset').options].some(o=>o.value===settings.preset)?settings.preset:'50x40';$('labelCustomWidth').value=settings.customWidth||settings.width||50;$('labelCustomHeight').value=settings.customHeight||settings.height||40;$('labelColumns').value=settings.columns||1;$('labelGap').value=settings.gap??2;$('labelPageMargin').value=settings.pageMargin??0;$('labelCodeMode').value=settings.codeMode||'both';$('labelShowName').checked=settings.showName!==false;$('labelShowPrice').checked=Boolean(settings.showPrice);$('labelShowProductCode').checked=settings.showProductCode!==false;$('labelShowBarcodeText').checked=Boolean(settings.showBarcodeText);document.querySelectorAll('.sp-custom-label-size').forEach(field=>{field.hidden=$('labelSizePreset').value!=='CUSTOM'});applyLabelSettings(false);}
  ...
  function bind() {
    ...
    ['labelPrinterMode','labelPrinterDpi','labelColumns','labelGap','labelPageMargin','labelCodeMode','labelShowName','labelShowPrice','labelShowProductCode','labelShowBarcodeText','labelCustomWidth','labelCustomHeight'].forEach((id) => {

(() => {
  ...
  async function drawBoxQr(box, quantity) {
    ...
    area.innerHTML = `<h3>ปิดกล่องสำเร็จ</h3>
      ...
      <div class="sp-actions"><button id="printBoxQrBtn" type="button" class="sp-primary">พิมพ์ QR กล่อง</button><a class="sp-primary" href="./stock-intake.html?scan=${encodeURIComponent(box.code)}">ส่งตรวจรับเข้าสต็อก →</a></div>`;
  ...
  async function closeBox() {
    ...
    if (client) {
      try {
        ...
        const waitingResult = await client.rpc('tkn_v5300_close_box_to_stock_intake', {
          p_box_code: box.code,
          p_category_text: box.mixedCategory ? (box.mixedCategories||[]).join(', ') : (box.category || identity.categoryLabel || null),
          p_zone_code: box.zoneCode || identity.zone || null,
          p_location_text: box.location || null,
        });
        if (waitingResult.error) throw waitingResult.error;
        box.historyId = waitingResult.data?.history_id || box.historyId || null;
    ...
    if (qrReady) message('ปิดกล่องแล้ว · ส่งเข้าคิวตรวจรับสต็อกอัตโนมัติ (WAITING_STOCK)', 'success');
  }

(() => {
  ...
  async function ensureBoxWaitingStock(box,{silent=false}={}) {
    ...
    // กล่องและรายการต้องถูกบันทึกก่อนค่อยสร้าง WAITING_STOCK snapshot
    const boxRow=await client.from('stock_boxes').select('id,box_code,status').eq('box_code',box.code).maybeSingle();
    if(boxRow.error) throw boxRow.error;
    if(!boxRow.data?.id) throw new Error('BOX_NOT_SAVED');

    const itemCheck=await client.from('stock_box_items').select('id',{count:'exact',head:true}).eq('box_id',boxRow.data.id).gt('quantity',0);
    if(itemCheck.error) throw itemCheck.error;
    if(!Number(itemCheck.count||0)) throw new Error('BOX_ITEMS_NOT_SAVED');

    const result=await client.rpc('tkn_v5300_close_box_to_stock_intake',{
      p_box_code:box.code,
    ...
    if(!result.data?.history_id || result.data?.workflow_status!=='WAITING_STOCK'){
      throw new Error('WAITING_STOCK_NOT_CONFIRMED');
    }

    box.historyId=result.data.history_id;

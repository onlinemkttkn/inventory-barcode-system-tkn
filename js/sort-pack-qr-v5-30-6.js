(() => {
  ...
  async function ensureBoxWaitingStock(box,{silent=false}={}) {
    ...
    const result=await client.rpc('tkn_v5306_recover_closed_box_to_waiting',{
      p_box_code:box.code,
    ...
    if (box.status === 'CLOSED') {
      try{
        ...
        return message(box.workflowStatus==='WAITING_STOCK'
          ? 'กล่องนี้ปิดแล้วและอยู่ในคิวตรวจรับสต็อก'
          : 'กล่องนี้รับเข้าสต็อกแล้ว','success');
      }catch(error){
        box.waitingStockSyncError=error.message||String(error);saveState();
        setQueueSyncStatus('bad',`กล่องปิดแล้วแต่ยังไม่เข้าคิว: ${box.waitingStockSyncError}`,{retry:true});
        await drawBoxQr(box,quantity);
        return message(`ยังไม่เข้าคิวตรวจรับ: ${box.waitingStockSyncError}`,'error');
      }
    }
    const categories=resolvedBoxCategories();if(categories.length>1){const confirmed=await confirmMixedCategory(categories);if(!confirmed)return message('ยกเลิกการปิดกล่อง · กรุณาแยกสินค้าเป็นคนละกล่อง','info');box.mixedCategory=true;box.mixedCategories=categories.map(row=>row.label);box.category='คละประเภท';}else{box.mixedCategory=false;box.mixedCategories=[];}const client=await waitClient();let identity;
    try {
    ...
    if (client) {
      try {
        ...
        const boxResult = await client.from('stock_boxes').upsert({
          ...
          location_text: box.location || null,
          source: 'SORT_PACK',
        ...
        // จุด Critical: ต้องยืนยันว่ามี History WAITING_STOCK จริงก่อนถือว่าปิด Flow สำเร็จ
        await ensureBoxWaitingStock(box,{silent:true});
        ...
        saveState();
        setQueueSyncStatus('bad',`กล่องถูกบันทึกแล้ว แต่ยังไม่เข้าคิวตรวจรับ: ${box.waitingStockSyncError}`,{retry:true});
        message(`ยังไม่ส่งเข้าคิวตรวจรับ · กด “ส่งเข้าคิวตรวจรับอีกครั้ง”: ${box.waitingStockSyncError}`, 'error');
        console.error('Close box WAITING_STOCK sync:',error);
      }
    }

    saveState();
    const qrReady = await drawBoxQr(box, quantity);
    if (qrReady && box.workflowStatus==='WAITING_STOCK') {
      setQueueSyncStatus('good',`กล่อง ${box.code} อยู่ในคิว WAITING_STOCK แล้ว`);
      message('ปิดกล่องแล้ว · ส่งเข้าคิวตรวจรับสต็อกสำเร็จ (WAITING_STOCK)', 'success');
    } else if (qrReady && box.workflowStatus!=='WAITING_STOCK') {
      setQueueSyncStatus('bad','QR สร้างแล้ว แต่ยังห้ามไป Stock Intake จนกว่าส่ง WAITING_STOCK สำเร็จ',{retry:true});
    }
  }

(() => {
  ...
  async function ensureBoxProducts(client,box) {
    for(const item of box.items||[]){
      ...
      const productResult=await client.rpc('ensure_sorting_product_lot_v5250',{
        p_sku:item.sku,
        ...
        p_source_barcode:item.sourceBarcode||null,
        p_product_type_th:item.subCategory||null,
        p_model_name:item.model||null,
      });
      if(productResult.error) throw productResult.error;
      item.productId=productResult.data?.id||null;
      if(!item.productId) throw new Error(`PRODUCT_ID_NOT_CREATED:${item.sku}`);
    }
  }
  ...
  async function ensureBoxWaitingStock(box,{silent=false}={}) {
    ...
    const result=await client.rpc('tkn_v5308_commit_box_to_waiting',{
      p_box_code:box.code,
      p_items:items,

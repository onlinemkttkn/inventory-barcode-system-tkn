(() => {
  ...
  if (!BOX_CODE) throw new Error('ไม่พบ TKN Box Code v5.25.6');

  const VERSION = '5.30.2';
  const LABEL_SETTINGS_VERSION = 13;
  const STATE_KEY = 'tkn_sort_pack_v5202';
  const LEGACY_STATE_KEY = 'tkn_sort_pack_v5201';
  const ENGINE = window.TKNCategoryEngine;
  if (!ENGINE) throw new Error('ไม่พบ TKN Category Engine v5.25.0');
  ...
  const DEFAULT_LABEL_SETTINGS=Object.freeze({printerMode:'AUTO',dpi:300,preset:'50x40',customWidth:50,customHeight:40,columns:1,gap:2,pageMargin:0,codeMode:'both',showName:true,showPrice:false,showProductCode:true,showBarcodeText:false,...LABEL_PROFILES['50x40']});
  ...
  const currentBranchId = () => {
    const value = sessionStorage.getItem('tkn_inventory_branch_id') || '';
  ...
  const compactCost = (value) => PATTERN?.compactCost?.(value) || String(Number(value || 0));
  const randomLetter = () => PATTERN?.randomLetter?.() || String.fromCharCode(65 + Math.floor(Math.random() * 26));
  const skuWithHiddenCost = (item) => {
    const current = String(item?.sku || item?.sourceSku || '').trim();
    if (!item?.hasCost || !current) return current;
    if (PATTERN?.parseLotSku?.(current)?.hasEmbeddedCost) return current;
    try { return PATTERN.buildLotSku(current, item.unitCost, item.costMaskLetter || 'X'); }
  ...
  function conciseItemName(item) {
    return PATTERN?.conciseLabel?.({
      name: item?.name,
      product_type_th: item?.subCategory,
      brand_name: item?.brand,
      model_name: item?.model,
    }, { maxChars: 52, maxLines: 2 }) || item?.name || '';
  }
  ...
  function migrateItem(item) {
    ...
    return {
      ...
      sourceAllocations: Array.isArray(item.sourceAllocations) ? item.sourceAllocations : ((item.sourceItemId || item.id) ? [{ id: item.sourceItemId || item.id, expectedQuantity: Math.max(1, numberValue(item.quantity, 1)) }] : []),
  ...
  function loadState() {
    try {
      ...
      if (state.box) {
        const parsedBox = BOX_CODE.parse(state.box.code);
        state.box.zoneCode = BOX_CODE.normalizeZone(state.box.zoneCode || parsedBox?.zone || 'A');
        state.box.shortCode = state.box.shortCode || parsedBox?.short || '';
        state.box.categoryCode = state.box.categoryCode || parsedBox?.categoryCode || '';
        state.box.category = state.box.category || parsedBox?.categoryLabel || 'คละประเภท';
        state.box.draftCode = state.box.draftCode || (parsedBox ? null : state.box.code);
      }
  ...
  function normalizeSourceRow(row, fallbackSource, tracking) {
    ...
    const mainCategory = String(row.main_category || row.category_th || row.raw_data?.main_category || '').trim();
    const subCategory = String(row.sub_category || row.subcategory_th || row.raw_data?.sub_category || '').trim();
    ...
    return migrateItem({
      ...
      sourceAllocations: [{ id: row.id || row.key || `${source}:${tracking}:${sourceSku}`, expectedQuantity: Math.max(1, numberValue(row.quantity ?? row.expected_quantity ?? row.item_quantity ?? row.actual_qty, 1)) }],
      source,
      tracking: String(row.tracking_number || tracking || '').trim(),
      order: String(row.order_number || row.return_number || '').trim(),
      sourceSku,
      sku,
      barcode: String(row.barcode || row.raw_data?.barcode || '').trim(),
      name: String(row.product_name || row.item_name || row.name || 'สินค้าไม่ระบุชื่อ').trim(),
      quantity: row.quantity ?? row.expected_quantity ?? row.item_quantity ?? row.actual_qty ?? 1,
      unitCost: costRaw,
      hasCost,
      sourcePrice: row.source_item_price ?? row.item_price_pp ?? row.selling_price ?? 0,
  ...
  function deduplicateItems(items) {
    ...
    for (const item of items) {
      ...
      existing.sourceAllocations = [...(existing.sourceAllocations || []), ...(item.sourceAllocations || [])];
  ...
  async function readV5271BatchRows(tracking) {
    const client = await waitClient();
    if (!client) return [];
    try {
      const selected = $('sourceSelect').value;
      const { data, error } = await client.rpc('tkn_v5271_find_sorting_source', {
        p_lookup: tracking,
        p_source: selected === 'AUTO' ? null : selected,
      });
      if (error) {
        if (!/function .* does not exist|Could not find the function/i.test(error.message || '')) console.warn('อ่านชุดงาน Marketplace v5.27.1 ไม่สำเร็จ:', error);
        return [];
      }
      const rows = Array.isArray(data) ? data : (data?.items || []);
      return rows.map((row) => normalizeSourceRow(row, row.source || selected, tracking));
    } catch (error) {
      console.warn('อ่านชุดงาน Marketplace v5.27.1 ไม่สำเร็จ:', error);
      return [];
    }
  }

  async function readServerRows(tracking) {
    const v5271Rows = await readV5271BatchRows(tracking);
    if (v5271Rows.length) return deduplicateItems(v5271Rows);
  ...
  async function persistItems() {
    ...
    await syncMarketplaceBatchProgress(state.items, false, null);
  }

  async function syncMarketplaceBatchProgress(items, stored = false, boxCode = null) {
    const client = await waitClient();
    if (!client) return;
    const payload = [];
    for (const item of items) {
      const allocations = Array.isArray(item.sourceAllocations) && item.sourceAllocations.length
        ? item.sourceAllocations
        : [{ id: item.sourceItemId, expectedQuantity: Number(item.quantity || 0) }];
      let remaining = Math.max(0, Number(item.quantity || 0));
      allocations.forEach((allocation, index) => {
        const expected = Math.max(0, Number(allocation.expectedQuantity || 0));
        const quantity = index === allocations.length - 1 ? remaining : Math.min(expected, remaining);
        remaining = Math.max(0, remaining - quantity);
        payload.push({
          batch_item_id: allocation.id,
          counted_quantity: quantity,
          stored_quantity: stored ? quantity : null,
          internal_sku: item.sku || item.sourceSku || null,
          product_id: item.productId || null,
          unit_cost: item.hasCost ? Number(item.unitCost || 0) : null,
          selling_price: Number(item.sellingPrice || 0) || null,
          category_th: item.category || item.mainCategory || null,
          subcategory_th: item.subCategory || null,
          box_code: boxCode,
        });
      });
    }
    const validPayload = payload.filter((row) => /^[0-9a-f-]{36}$/i.test(String(row.batch_item_id || '')));
    if (!validPayload.length) return;
    const { error } = await client.rpc('tkn_v5271_sync_sorting_progress', { p_items: validPayload, p_mark_stored: stored });
    if (error && !/function .* does not exist|Could not find the function/i.test(error.message || '')) {
      console.warn('อัปเดตความคืบหน้าชุดงาน Marketplace ไม่สำเร็จ:', error);
    }
  }
  ...
  async function loadTracking() {
    ...
    try {
      ...
      if (!items.length && selectedSource !== 'MANUAL') {
        const alertText = `ไม่พบสินค้าในไฟล์งาน\n\nรหัสที่สแกน: ${tracking}\nกรุณาตรวจสอบ SKU / Barcode หรือไฟล์นำเข้าก่อน`;
        message(`ไม่พบสินค้าในไฟล์งาน: ${tracking} · รายการนี้ยังไม่ถูกเพิ่ม`, 'error');
      ...
      if (!items.length) {
        ...
        message('โหมดรับเข้าเอง: สร้างรายการรอระบุข้อมูล 1 รายการ', 'info');
      }
      ...
      if (selectedSource !== 'MANUAL') {
        const unresolvedItems = items.filter((item) => {
          ...
          return !name || /^(สินค้าไม่ระบุชื่อ|รายการใหม่)/i.test(name);
        });
        if (unresolvedItems.length) {
          const unresolvedCodes = unresolvedItems
            .map((item) => String(item?.sku || item?.sourceSku || item?.barcode || tracking).trim())
            .filter(Boolean)
            .slice(0, 5)
            .join(', ');
          const suffix = unresolvedItems.length > 5 ? ` และอีก ${unresolvedItems.length - 5} รายการ` : '';
          const alertText = `ไม่พบสินค้าในไฟล์งาน\n\nรหัสที่สแกน: ${tracking}${unresolvedCodes ? `\nรายการที่ไม่พบชื่อสินค้า: ${unresolvedCodes}${suffix}` : ''}\nกรุณาตรวจสอบไฟล์นำเข้าก่อน`;
          message(`ไม่พบชื่อสินค้าในไฟล์งาน: ${tracking} · รายการนี้ยังไม่ถูกเพิ่ม`, 'error');
          if (typeof window.alert === 'function') window.alert(alertText);
          $('trackingInput').select();
          return;
        }
      ...
      items.forEach((incoming) => {
        ...
        if (existing) {
          ...
          existing.sourceAllocations = [...(existing.sourceAllocations || []), ...(incoming.sourceAllocations || [])];
          increased += 1;
        } else {
  ...
  function renderBox() {
    if(state.box?.workflowStatus==='WAITING_STOCK'){
      setQueueSyncStatus('good',`กล่อง ${state.box.code} อยู่ในคิว WAITING_STOCK แล้ว`);
    }else if(state.box?.waitingStockSyncError){
      setQueueSyncStatus('bad',`ยังไม่เข้าคิวตรวจรับ: ${state.box.waitingStockSyncError}`,{retry:true});
    }
    ...
    const itemCategories = unique(box.items.map((item) => item.category));
    if (itemCategories.length === 1 && itemCategories[0]) box.category = itemCategories[0];
    $('boxCode').value = box.shortCode || BOX_CODE.shortCode(box.code) || box.code;
    $('boxLocation').value = box.location || '';
    $('boxZoneCode').value = BOX_CODE.normalizeZone(box.zoneCode || 'A');
    ...
    $('boxContents').innerHTML = box.items.map((item) => `<article class="sp-item done" data-id="${esc(item.id)}">
      <div class="sp-qty"><b>${item.quantity}</b><small>ชิ้น</small></div>
      <div class="sp-item-main"><h3>${esc(conciseItemName(item))}</h3><p><b>SKU ${esc(item.sku)}</b> · ต้นทุน ฿${money(item.unitCost)}${item.sellingPrice ? ` · ราคาขาย ฿${money(item.sellingPrice)}` : ''} · ${esc(item.category)} · (${codeCost(item.unitCost)})</p></div>
    ...
    const ls=state.labelSettings||DEFAULT_LABEL_SETTINGS,usesQr=ls.codeMode==='qr'||ls.codeMode==='both',usesBarcode=ls.codeMode==='barcode'||ls.codeMode==='both';
    $('labelPreview').innerHTML=units.map(({item,index,quantity})=>`<article class="sp-label tkn-label-product" data-label="${esc(item.id)}" data-unit="${index}"><div class="sp-label-qr-status" hidden></div>${usesQr?'<canvas class="sp-label-qr tkn-label-qr"></canvas>':''}${usesBarcode?`<div class="tkn-label-barcode-holder"><svg class="sp-label-barcode tkn-label-barcode" aria-label="Barcode ${esc(item.sku)}"></svg>${ls.showBarcodeText?`<small>${esc(ensureLotSku(item))}</small>`:''}</div>`:''}${ls.showProductCode?`<small class="tkn-label-sku">${esc(ensureLotSku(item))}${quantity>1?` · ${index}/${quantity}`:''}</small>`:''}${ls.showName?`<h4 class="tkn-label-name">${esc(conciseItemName(item))}</h4>`:''}${ls.showPrice&&Number(item.sellingPrice||0)>0?`<strong class="tkn-label-price">฿${money(item.sellingPrice)}</strong>`:''}</article>`).join('');
  ...
  async function drawLabels() {
    ...
    await Promise.all(elements.map(async (element) => {
      ...
      if (!item) return;
      try {
        const dpi=Math.max(203,Number(state.labelSettings?.dpi)||300),pixelSize=Math.min(2048,Math.max(96,Math.round((Number(state.labelSettings?.qr||20)/25.4)*dpi)));ensureLotSku(item);
        if(canvas){if(!ready||typeof toCanvas!=='function')throw new Error('ระบบ QR ยังไม่พร้อม');await toCanvas(canvas,PATTERN?.qrValue?.(item.sku)||`TKN-P-${item.sku}`,{width:pixelSize,margin:1,errorCorrectionLevel:'M'});}
        if(barcodeSvg){if(typeof window.JsBarcode!=='function')throw new Error('ระบบ Barcode ยังไม่พร้อม');const barcodeValue=PATTERN?.barcodeValue?.(item.sku)||String(item.sku||'').trim();if(!barcodeValue)throw new Error('สินค้าไม่มี Barcode หรือ SKU');const barWidth=barcodeValue.length>24?.65:barcodeValue.length>16?.85:1.15;window.JsBarcode(barcodeSvg,barcodeValue,{format:'CODE128',displayValue:false,margin:0,width:barWidth,height:Math.max(18,Number(state.labelSettings?.barcodePx||window.TKNLabelLayout?.mmToPx?.(state.labelSettings?.barcodeHeight||6,state.labelSettings?.dpi||300)||34)),background:'#fff',lineColor:'#000'});}
  ...
  function labelProfile(preset, customWidth, customHeight) {
    ...
    if (unified?.productProfile) {
      return unified.productProfile({ preset, customWidth, customHeight, dpi: Number(state.labelSettings?.dpi) || 300 });
    }
    ...
    const height = Math.max(10, Math.min(150, numberValue(customHeight, 40)));
    ...
    return {
      ...
      nameWeight: 800, skuWeight: 850, nameLines: shortSide >= 35 ? 2 : 1,
    };
  }

  function readLabelSettings(){const preset=$('labelSizePreset')?.value||'50x40',customWidth=numberValue($('labelCustomWidth')?.value,50),customHeight=numberValue($('labelCustomHeight')?.value,40),dpi=[203,300,600].includes(Number($('labelPrinterDpi')?.value))?Number($('labelPrinterDpi').value):300,profile=labelProfile(preset,customWidth,customHeight);return{printerMode:$('labelPrinterMode')?.value||'AUTO',dpi,preset,customWidth,customHeight,columns:Math.max(1,Math.min(8,Math.round(numberValue($('labelColumns')?.value,1)))),columnsMode:$('labelColumns')?.dataset.userEdited==='1'?'manual':'recommended',gap:Math.max(0,Math.min(20,numberValue($('labelGap')?.value,2))),pageMargin:Math.max(0,Math.min(30,numberValue($('labelPageMargin')?.value,0))),codeMode:$('labelCodeMode')?.value||'both',showName:Boolean($('labelShowName')?.checked),showPrice:Boolean($('labelShowPrice')?.checked),showProductCode:Boolean($('labelShowProductCode')?.checked),showBarcodeText:Boolean($('labelShowBarcodeText')?.checked),...profile};}

  function applyRecommendedLabelColumns({force=false}={}) {
    const preset=$('labelSizePreset')?.value;
    const recommended=window.TKNLabelLayout?.recommendedColumns?.(preset,null);
    if(!recommended){
      if($('labelRowPresetHint')) $('labelRowPresetHint').textContent='Preset เดิมใช้จำนวนต่อแถวเดิม';
      return;
    }
    const input=$('labelColumns');
    if(input && (force || !input.dataset.userEdited)) input.value=String(recommended);
    const profile=labelProfile(preset,$('labelCustomWidth')?.value,$('labelCustomHeight')?.value);
    const micro=profile?.density==='micro'?' · Micro: ซ่อนชื่อ/ราคาอัตโนมัติเพื่อไม่ให้ข้อมูลทับกัน':'';
    if($('labelRowPresetHint')) $('labelRowPresetHint').textContent=`แนะนำ ${recommended} ต่อแถว${micro} · แก้จำนวนเองได้`;
  }
  ...
  function applyLabelSettings(save = true) {
    ...
    if (grid) {
      ...
      window.TKNLabelLayout?.applyProductVars?.(grid, profile);
    }
    ...
    const rollWidth=window.TKNPrintPlatform?.rowPhysicalWidth?.(settings,width)||(Number(settings.columns||1)*width+Math.max(0,Number(settings.columns||1)-1)*Number(settings.gap||0)+2*Number(settings.pageMargin||0));
    printStyle.textContent=printMode==='ROLL'?`@page{size:${rollWidth}mm ${height}mm;margin:${Number(settings.pageMargin||0)}mm}`:`@page{size:A4 portrait;margin:${Number(settings.pageMargin||0)}mm}`;
    ...
    if(save){window.TKNPrintPlatform?.saveProductSettings?.(settings);saveState();renderBox();}
  }

  function fillLabelSettingInputs(){const shared=window.TKNPrintPlatform?.getProductSettings?.()||{},settings={...DEFAULT_LABEL_SETTINGS,...(state.labelSettings||{}),...shared};state.labelSettings=settings;if(!$('labelSizePreset'))return;$('labelPrinterMode').value=settings.printerMode||'AUTO';$('labelPrinterDpi').value=String(settings.dpi||300);$('labelSizePreset').value=[...$('labelSizePreset').options].some(o=>o.value===settings.preset)?settings.preset:'50x40';$('labelCustomWidth').value=settings.customWidth||settings.width||50;$('labelCustomHeight').value=settings.customHeight||settings.height||40;$('labelColumns').value=settings.columns||1;$('labelColumns').dataset.userEdited=settings.columnsMode==='manual'?'1':'';$('labelGap').value=settings.gap??2;$('labelPageMargin').value=settings.pageMargin??0;$('labelCodeMode').value=settings.codeMode||'both';$('labelShowName').checked=settings.showName!==false;$('labelShowPrice').checked=Boolean(settings.showPrice);$('labelShowProductCode').checked=settings.showProductCode!==false;$('labelShowBarcodeText').checked=Boolean(settings.showBarcodeText);document.querySelectorAll('.sp-custom-label-size').forEach(field=>{field.hidden=$('labelSizePreset').value!=='CUSTOM'});applyRecommendedLabelColumns({force:false});applyLabelSettings(false);}

  function resolvedBoxCategories() {
    return BOX_CODE.categoriesFromItems(state.box?.items || []);
  }

  function boxCodePreview() {
    const box = ensureBox();
    if (!box) return '-';
    const categories = resolvedBoxCategories();
    const category = categories.length === 1 ? categories[0] : BOX_CODE.resolveCategory(box.category || 'อื่น ๆ');
    return box.shortCode || BOX_CODE.preview(category.label, box.zoneCode || 'A');
  }

  function confirmMixedCategory(categories){
    const labels=(categories||[]).map(row=>row.label).filter(Boolean),dialog=$('mixedCategoryDialog');
    if(!dialog?.showModal) return Promise.resolve(confirm(`กล่องนี้มีสินค้าต่างประเภท (${labels.join(', ')})\n\nแนะนำให้แยกสินค้าเป็นคนละกล่อง แต่สามารถยืนยันปิดแบบคละประเภทเพื่อใช้พื้นที่กล่องให้เต็มได้\n\nยืนยันปิดกล่องคละประเภทหรือไม่?`));
    $('mixedCategoryList').innerHTML=labels.map(label=>`<li>${esc(label)}</li>`).join('');
    return new Promise(resolve=>{
      const cleanup=()=>{$('mixedCategoryConfirmBtn').removeEventListener('click',yes);$('mixedCategoryBackBtn').removeEventListener('click',no);dialog.removeEventListener('cancel',cancel);};
      const done=value=>{cleanup();try{dialog.close()}catch{}resolve(value)},yes=()=>done(true),no=()=>done(false),cancel=e=>{e.preventDefault();done(false)};
      $('mixedCategoryConfirmBtn').addEventListener('click',yes);$('mixedCategoryBackBtn').addEventListener('click',no);dialog.addEventListener('cancel',cancel);dialog.showModal();
    });
  }


  function setQueueSyncStatus(kind,text,{retry=false}={}) {
    const panel=$('boxQueueSyncPanel'), title=$('boxQueueSyncTitle'), body=$('boxQueueSyncText'), btn=$('retryWaitingStockBtn');
    if(!panel) return;
    panel.hidden=false;
    panel.className=`sp-sync-panel ${kind==='good'?'good':kind==='bad'?'bad':''}`;
    if(title) title.textContent=kind==='good'?'ส่งเข้าคิวตรวจรับแล้ว':'ยังไม่เข้าคิวตรวจรับ';
  ...
  async function ensureBoxProducts(client,box) {
    for(const item of box.items||[]){
      ...
      const productResult=await client.rpc('tkn_v5309_resolve_or_create_sorting_product',{
        p_sku:item.sku,
        p_name:safeProductName,
        p_cost_price:Number(item.unitCost||0),
        p_selling_price:Number(item.sellingPrice||0),
        p_source_barcode:item.sourceBarcode||item.barcode||null,
        p_product_type_th:item.subCategory||item.category||null,
        p_model_name:item.model||null,
        p_lot_code:item.lotCode||state.lot?.id||null,
        p_lot_cost_letter:item.costLetter||null,
      });
  ...
  async function ensureBoxWaitingStock(box,{silent=false}={}) {
    if(!box?.code) throw new Error('BOX_CODE_REQUIRED');
    if(!box?.items?.length) throw new Error('BOX_ITEMS_REQUIRED');
    const client=await waitClient();
    if(!client?.rpc) throw new Error('SUPABASE_CLIENT_NOT_READY');

    await ensureBoxProducts(client,box);
    const items=atomicBoxItems(box);
    if(items.length!==box.items.length || items.some(item=>!item.product_id)){
      throw new Error('BOX_ITEMS_NOT_READY_FOR_DATABASE');
    }

    const result=await client.rpc('tkn_v5309_commit_box_to_waiting',{
      p_box_code:box.code,
      p_items:items,
      p_category_text:box.mixedCategory ? (box.mixedCategories||[]).join(', ') : (box.category||null),
      p_zone_code:box.zoneCode||null,
      p_location_text:box.location||null,
    });
    if(result.error) throw result.error;
    const data=result.data||{};

    if(data.workflow_status==='IN_STOCK'){
      box.historyId=data.history_id||box.historyId||null;
      box.workflowStatus='IN_STOCK';
      box.status='CLOSED';
      box.waitingStockSyncError=null;
      saveState();
      if(!silent) setQueueSyncStatus('good',`กล่อง ${data.box_code||box.code} รับเข้าสต็อกแล้ว`);
      return data;
    }

    if(!data.history_id || data.workflow_status!=='WAITING_STOCK'){
      throw new Error('WAITING_STOCK_NOT_CONFIRMED');
    }

    box.code=data.box_code||box.code;
    box.historyId=data.history_id;
    box.workflowStatus='WAITING_STOCK';
    box.status='CLOSED';
    box.closedAt=box.closedAt||nowIso();
    box.waitingStockSyncedAt=new Date().toISOString();
    box.waitingStockSyncError=null;
    saveState();
    if(!silent) setQueueSyncStatus('good',`กล่อง ${box.code} อยู่ในคิว WAITING_STOCK แล้ว`);
    return data;
  }

  async function retryCurrentBoxWaitingStock() {
    const box=state.box;
    if(!box?.code) return message('ไม่พบกล่องปัจจุบัน','error');
    const btn=$('retryWaitingStockBtn');
    if(btn) btn.disabled=true;
    try{
      setQueueSyncStatus('',`กำลังส่งกล่อง ${box.code} เข้าคิวตรวจรับ...`);
      await ensureBoxWaitingStock(box);
      message('ส่งเข้าคิวตรวจรับสต็อกสำเร็จ','success');
      renderBox();
    }catch(error){
      box.waitingStockSyncError=error.message||String(error); saveState();
      setQueueSyncStatus('bad',`ส่งเข้าคิวไม่สำเร็จ: ${box.waitingStockSyncError}`,{retry:true});
      message(`ยังไม่ส่งเข้าคิวตรวจรับ: ${box.waitingStockSyncError}`,'error');
      console.error('WAITING_STOCK retry:',error);
    }finally{
      if(btn) btn.disabled=false;
    }
  }

  async function finalizeBoxIdentity(client) {
    const box = ensureBox();
    const categories = resolvedBoxCategories();
    let category=categories.length>1?BOX_CODE.resolveCategory('คละประเภท'):(categories[0]||BOX_CODE.resolveCategory(box.category||''));
    if (category.code === 'GEN' && box.category && box.category !== 'คละประเภท') category = BOX_CODE.resolveCategory(box.category);
    if (!category?.label || box.category === 'คละประเภท' && !categories.length) {
      throw new Error('กรุณาเลือกประเภทกล่องก่อนปิด');
    }
    const zone = BOX_CODE.normalizeZone(box.zoneCode || $('boxZoneCode')?.value || 'A');
    const oldCode = box.code;
    const identity = await BOX_CODE.nextIdentity({ category: category.label, zone, currentCode: box.code, client });
    box.draftCode = box.draftCode || (BOX_CODE.parse(oldCode) ? null : oldCode);
    box.code = identity.canonical;
    box.shortCode = identity.short;
    box.category = identity.categoryLabel;
    box.categoryCode = identity.categoryCode;
    box.zoneCode = identity.zone;
    return identity;
  }
  ...
  function renderClose() {
    ...
    const categoryText=categories.map(row=>row.label).join(', ')||state.box.category||'-',mixedNote=categories.length>1?`<div class="tkn-mixed-box-note">⚠ กล่องนี้มี ${categories.length} ประเภท: ${esc(categoryText)} — ระบบจะเตือนอีกครั้งก่อนปิด แต่สามารถปิดเป็นกล่องคละประเภทเพื่อใช้พื้นที่ให้เต็มได้</div>`:'';$('closeSummary').innerHTML=`${mixedNote}<div class="sp-summary-grid">
      <article><span>รหัสเมื่อปิด</span><b>${esc(boxCodePreview())}</b></article>
      <article><span>ประเภท</span><b>${esc(categoryText)}</b></article>
      <article><span>โซน</span><b>${esc(BOX_CODE.normalizeZone(state.box.zoneCode || 'A'))}</b></article>
      <article><span>SKU</span><b>${state.box.items.length}</b></article>
      <article><span>จำนวน</span><b>${quantity}</b></article>
      <article><span>ต้นทุนรวม</span><b>฿${money(totalCost)}</b></article>
    </div>
    <p><strong>รหัสเต็มใน QR:</strong> ${esc(state.box.shortCode ? state.box.code : `TKN-B-${boxCodePreview()}`)}</p>
  ...
  async function drawBoxQr(box, quantity) {
    ...
    area.innerHTML = `<h3>ปิดกล่องสำเร็จ</h3>
      ...
      <p class="sp-box-code"><b>${esc(box.shortCode || BOX_CODE.shortCode(box.code))}</b></p>
      <canvas id="boxQrCanvas" aria-label="QR กล่อง ${esc(box.code)}"></canvas>
      <p><strong>${esc(box.category || 'อื่น ๆ')}</strong> · โซน ${esc(box.zoneCode || 'A')}</p>
      <p>${box.items.length} SKU · ${quantity} ชิ้น</p>
      <div class="sp-actions"><button id="printBoxQrBtn" type="button" class="sp-primary">พิมพ์ QR กล่อง</button>${box.workflowStatus==='WAITING_STOCK'?`<a class="sp-primary" href="./stock-intake.html?scan=${encodeURIComponent(box.code)}">ไปหน้าตรวจรับเข้าสต็อก →</a>`:`<button id="retryWaitingStockInlineBtn" type="button" class="sp-primary">ส่งเข้าคิวตรวจรับก่อน →</button>`}</div>`;
  ...
  async function closeBox() {
    ...
    if(box.status==='CLOSED'){
      try{
        ...
        return message(box.workflowStatus==='WAITING_STOCK'
          ?'กล่องนี้อยู่ในคิวตรวจรับสต็อกแล้ว'
          :'กล่องนี้รับเข้าสต็อกแล้ว','success');
      }catch(error){
        box.waitingStockSyncError=error.message||String(error);saveState();
        setQueueSyncStatus('bad',`กล่องยังไม่ถูกบันทึกครบ: ${box.waitingStockSyncError}`,{retry:true});
        return message(`ยังส่งเข้าคิวไม่ได้: ${box.waitingStockSyncError}`,'error');
      }
    }

    const categories=resolvedBoxCategories();
    if(categories.length>1){
      const confirmed=await confirmMixedCategory(categories);
      if(!confirmed) return message('ยกเลิกการปิดกล่อง · กรุณาแยกสินค้าเป็นคนละกล่อง','info');
      box.mixedCategory=true;
      box.mixedCategories=categories.map(row=>row.label);
      box.category='คละประเภท';
    }else{
      box.mixedCategory=false;
      box.mixedCategories=[];
    }
    ...
    let identity;
    try{
      identity=await finalizeBoxIdentity(client);
    }catch(error){
      return message(error.message||'สร้างรหัสกล่องไม่สำเร็จ','error');
    }

    if(!box.mixedCategory && !confirm(`ปิดผนึกกล่อง ${identity.short} · ${identity.categoryLabel} · โซน ${identity.zone} · ${quantity} ชิ้น?`)) return;

    setQueueSyncStatus('',`กำลังบันทึกกล่อง ${box.code} และสร้างคิว WAITING_STOCK...`);
    try{
      // Critical path is one backend transaction. Do not mark CLOSED locally before this succeeds.
      const result=await ensureBoxWaitingStock(box,{silent:true});
      ...
      // Marketplace progress is auxiliary. It must never invalidate an already-created WAITING_STOCK box.
      try{
        await persistLot();
        await persistItems();
        await syncMarketplaceBatchProgress(box.items,true,box.code);
      }catch(auxError){
        console.warn('Marketplace auxiliary sync pending:',auxError);
        message(`กล่องเข้าคิวสต็อกแล้ว แต่ซิงก์สถานะ Marketplace บางส่วนยังไม่ครบ: ${auxError.message||auxError}`,'info');
      }

      const qrReady=await drawBoxQr(box,quantity);
      if(qrReady){
        setQueueSyncStatus('good',`กล่อง ${box.code} บันทึกฐานข้อมูลและอยู่ในคิว WAITING_STOCK แล้ว`);
        message('ปิดกล่องสำเร็จ · บันทึกฐานข้อมูลและส่งเข้าคิวตรวจรับสต็อกแล้ว','success');
      }
      return result;
    }catch(error){
      // Keep as draft so the user cannot mistake a browser-only QR for a committed box.
      box.status='DRAFT';
      box.workflowStatus='SYNC_ERROR';
      box.waitingStockSyncError=error.message||String(error);
      saveState();
      setQueueSyncStatus('bad',`ปิดกล่องไม่สำเร็จและยังไม่สร้าง QR: ${box.waitingStockSyncError}`,{retry:true});
      message(`ปิดกล่องไม่สำเร็จ: ${box.waitingStockSyncError}`,'error');
      console.error('Atomic close box:',error);
      return null;
    }
  }
  ...
  function bind() {
    document.addEventListener('click', (event) => {
      ...
      if (button.dataset.autoSku !== undefined && item) {
        ...
        item.costMaskLetter = item.costMaskLetter || randomLetter();
        item.sku = item.hasCost ? PATTERN.buildLotSku(baseSku, item.unitCost, item.costMaskLetter) : baseSku;
    ...
    $('labelSizePreset').addEventListener('change', (event) => {
      ...
      if($('labelColumns')) $('labelColumns').dataset.userEdited='';
      applyRecommendedLabelColumns({force:true});
      applyLabelSettings(true);
    });
    $('labelColumns')?.addEventListener('change',()=>{
      $('labelColumns').dataset.userEdited='1';
      applyLabelSettings(true);
    });
    ['labelPrinterMode','labelPrinterDpi','labelGap','labelPageMargin','labelCodeMode','labelShowName','labelShowPrice','labelShowProductCode','labelShowBarcodeText','labelCustomWidth','labelCustomHeight'].forEach((id) => {
    ...
    $('printLabelsBtn').addEventListener('click',async()=>{if(!state.labelQueue.length)return message('คิวฉลากว่าง','error');applyLabelSettings(false);await drawLabels();window.print();});$('downloadLabelsPngBtn')?.addEventListener('click',async()=>{try{applyLabelSettings(false);await drawLabels();await window.TKNPrintPlatform.downloadPng($('labelPreview'),'tkn-sort-pack-labels.png')}catch(error){message(error.message||'สร้าง PNG ไม่สำเร็จ','error')}});$('shareLabelsPngBtn')?.addEventListener('click',async()=>{try{applyLabelSettings(false);await drawLabels();await window.TKNPrintPlatform.sharePng($('labelPreview'),'tkn-sort-pack-labels.png')}catch(error){message(error.message||'แชร์ PNG ไม่สำเร็จ','error')}});
    ...
    $('boxCategory').addEventListener('change', (event) => { const box = ensureBox(); if (box) { box.category = event.target.value; saveState(); renderClose(); } });
    $('boxZoneCode').addEventListener('change', (event) => { const box = ensureBox(); if (box) { box.zoneCode = BOX_CODE.normalizeZone(event.target.value); saveState(); renderClose(); } });
  ...
  async function init() {
    ...
    const batchCode = query.get('batch');
    const batchSource = String(query.get('source') || '').toUpperCase();
    if (['SHOPEE','LAZADA','MANUAL'].includes(batchSource)) $('sourceSelect').value = batchSource;
    if (batchCode && !state.items.length) {
      $('lotNote').value = `ชุดงาน ${batchCode}`;
      message(`เปิดชุดงาน ${batchCode} แล้ว · กรุณาสแกน Tracking, SKU หรือ Barcode จากสินค้าจริงภายในกล่อง`, 'success');
      $('trackingInput').focus();
    }
  }

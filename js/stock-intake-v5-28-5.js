(() => {
  ...
  async function loadBranches() {
    ...
    const query=sb.from('branches').select('id,code,name,is_active').eq('is_active',true).order('name');
    const {data,error}=await timeout(query,8000,'โหลดรายชื่อสาขา');
    if(error) throw error;
    state.branches=data || [];
    const select=$('branchSelect');
    if(select) select.innerHTML=state.branches.map(b=>`<option value="${esc(b.id)}">${esc(b.code)} — ${esc(b.name||b.code)}</option>`).join('');
  }
  ...
  function renderDetail(data) {
    ...
    if(h.branch_id && state.branches.some(b=>String(b.id)===String(h.branch_id)) && $('branchSelect')) $('branchSelect').value=h.branch_id;
  ...
  async function receive() {
    ...
    try {
      const sb=await client();
      const {data,error}=await timeout(sb.rpc('tkn_v5285_receive_stock_intake',{p_history_id:state.detail.history.id,p_branch_id:$('branchSelect').value,p_scan_code:$('boxScan').value.trim(),p_note:$('intakeNote').value.trim()||null}),12000,'รับกล่องเข้าสต็อก');
      if(error) throw error;
      msg(data?.already_received?`กล่องนี้รับเข้าแล้ว · ${data.stock_document_no||'-'}`:`รับเข้าสต็อกสำเร็จ · เอกสาร ${data?.stock_document_no||'-'}`,'good');

(() => {
  ...
  async function loadQueue() {
    ...
    if($('queueRows')) $('queueRows').innerHTML=rows.map(r=>`<tr><td>${fmt(r.closed_at)}</td><td><b>${esc(r.box_code)}</b><br><small>${esc(r.category_text||'-')}${String(r.category_text||'').includes(',')?'<span class="intake-mixed-badge">คละประเภท</span>':''}</small></td><td>${esc(r.sku_count)} SKU · ${num(r.total_quantity)} ชิ้น</td><td>${esc(r.location_text||'-')}</td><td><button class="intake-btn" type="button" data-intake-box="${esc(r.box_code)}">ตรวจรับ</button></td></tr>`).join('') || '<tr><td colspan="5">ไม่มีกล่องรอเข้าสต็อก</td></tr>';
  }
  ...
  async function lookup(code) {
    const value=String(code||$('boxScan')?.value||'').trim();
    ...
    } catch(e) { state.detail=null; renderDetail(null); msg(e.message||String(e),'error'); console.error('Stock Intake lookup:',e); }
  ...
  async function receive() {
    ...
    try {
      ...
      if(error) throw error;
      msg(data?.already_received?`กล่องนี้รับเข้าแล้ว · ${data.stock_document_no||'-'}`:`รับเข้าสต็อกสำเร็จ · ${data?.stock_document_no||'-'} · รอขึ้นชั้นจัดเก็บ`,'good');
      if($('putawayNext')){ $('putawayNext').hidden=false; $('putawayNext').href=`./stock-putaway.html?scan=${encodeURIComponent(state.detail.history.box_code)}`; }

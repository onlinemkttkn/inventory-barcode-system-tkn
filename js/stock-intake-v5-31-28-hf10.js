(() => {
  ...
  async function resolveUserLabels(rows) {
    const ids=[...new Set((rows||[]).map(row=>String(row?.received_by||'').trim()).filter(value=>UUID_RE.test(value)))];
    const labels=new Map();
    if(!ids.length) return labels;
    try {
      const sb=await client();
      const {data,error}=await timeout(sb.rpc('tkn_v5310_user_labels',{p_user_ids:ids}),7000,'โหลดชื่อผู้ใช้งาน');
      if(error) throw error;
      for(const row of Array.isArray(data)?data:[]) {
        const id=String(row?.user_id||'').trim();
        if(!id) continue;
        const label=String(row?.label||row?.display_name||row?.email||'').trim();
        if(label) labels.set(id,label);
      }
    } catch(error) {
      console.warn('Stock Intake user labels:',error);
    }
    return labels;
  }
  ...
  async function loadBranches() {
    ...
    if(!wh?.id) throw new Error('ไม่พบสำนักงานใหญ่ / คลังหลัก');
    state.branches=[wh];
    const select=$('branchSelect');
    if(select){
      select.innerHTML=`<option value="${esc(wh.id)}">${esc(wh.name||'สำนักงานใหญ่ / คลังหลัก')}</option>`;
      select.value=wh.id;
  ...
  async function loadRecent() {
    ...
    if(error) throw error;
    const rows=Array.isArray(data)?data:[];
    const userLabels=await resolveUserLabels(rows);
    if($('recentRows')) $('recentRows').innerHTML=rows.map(r=>{
      const rawUser=String(r.received_by||'').trim();
      const label=userLabels.get(rawUser)||String(r.received_by_label||r.user_label||'').trim()||userFallback(rawUser);
      const title=UUID_RE.test(rawUser)?`User ID: ${rawUser}`:'';
      return `<tr><td>${fmt(r.received_at)}</td><td><b>${esc(r.box_code)}</b></td><td>${esc(r.stock_document_no||'-')}</td><td>${num(r.total_quantity)}</td><td>${esc(r.branch_name||'-')}</td><td${title?` title="${esc(title)}"`:''}><b>${esc(label)}</b></td></tr>`;
    }).join('')||'<tr><td colspan="6">ยังไม่มีประวัติรับเข้า</td></tr>';
  }

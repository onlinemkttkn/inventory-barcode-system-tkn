(() => {
  ...
  const PRODUCT_DEFAULTS=Object.freeze({printerMode:'AUTO',dpi:300,preset:'label58',customWidth:58,customHeight:38,columns:1,columnsMode:'recommended',gap:2,pageMargin:0,codeMode:'both',conciseName:true,showName:true,showPrice:false,showProductCode:true,showBarcodeText:false});
  ...
  function normalizeProduct(v={}){return {...PRODUCT_DEFAULTS,...v,dpi:[203,300,600].includes(Number(v.dpi))?Number(v.dpi):300,columns:Math.round(clamp(v.columns,1,8)),columnsMode:v.columnsMode==='manual'?'manual':'recommended',gap:clamp(v.gap,0,20),pageMargin:clamp(v.pageMargin,0,30),customWidth:clamp(v.customWidth,20,210),customHeight:clamp(v.customHeight,10,297),codeMode:['qr','both','barcode'].includes(v.codeMode)?v.codeMode:'both',conciseName:bool(v.conciseName,true),showName:bool(v.showName,true),showPrice:bool(v.showPrice,false),showProductCode:bool(v.showProductCode,true),showBarcodeText:bool(v.showBarcodeText,false)}}
  ...
  function rowPhysicalWidth(settings,width){
    const columns=Math.max(1,Math.round(Number(settings?.columns||1)));
    const gap=Math.max(0,Number(settings?.gap||0));
    const margin=Math.max(0,Number(settings?.pageMargin||0));
    return columns*Number(width||0)+Math.max(0,columns-1)*gap+2*margin;
  }
  function resolvePrintMode(settings,width){
    const mode=String(settings?.printerMode||'AUTO').toUpperCase();
    if(mode==='ROLL'||mode==='SHEET')return mode;
    const total=rowPhysicalWidth(settings,width);
    return total>0 && total<=110 ? 'ROLL' : 'SHEET';
  }

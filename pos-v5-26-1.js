...
const ids = [
  'branch','boxSaleLink','payment','customerName','customerPhone','searchForm','search','searchButton',
...
function canBoxSale(){
  ...
  return ['owner','admin'].includes(role)||(access?.permissions||[]).includes('pos.box_sale.create');
}
function refreshBoxSaleLink(){
  if(!E.boxSaleLink)return;
  E.boxSaleLink.hidden=!canBoxSale();
  E.boxSaleLink.href=`./pos-box-sale.html${hasBranch()?`?branch=${encodeURIComponent(E.branch.value)}`:''}`;
}
...
async function searchProducts(event){
  ...
  if(scan.kind==='BOX'){
    const target=canBoxSale()
      ? `./pos-box-sale.html?scan=${encodeURIComponent(scan.raw)}${hasBranch()?`&branch=${encodeURIComponent(E.branch.value)}`:''}`
      : `./mobile-stock-check.html?scan=${encodeURIComponent(scan.raw)}`;
    msg(E.searchMsg,canBoxSale()?'กำลังเปิด POS ขายยกกล่อง':'ไม่มีสิทธิ์ขายยกกล่อง ระบบกำลังเปิดหน้าตรวจสอบกล่อง','success');
...
E.branch.onchange=()=>{if(shift?.shift_id)return;cart.clear();E.results.innerHTML='';renderCart();refreshBoxSaleLink();if(hasBranch()){branchReady=true;sessionStorage.setItem('tkn_inventory_branch_id',E.branch.value);refreshPosAvailability(`พร้อมใช้งาน: ${E.branch.options[E.branch.selectedIndex]?.text||''}`);setShiftLockVisible(!shift?.shift_id)}};
installLogoutGuard();

...
function canBoxSale(){return Boolean(boxSaleAccess?.can_create)}
async function refreshBoxSaleAccess(){
  boxSaleAccess=null;
  if(E.boxSaleButton)E.boxSaleButton.hidden=true;
  if(!shift?.shift_id||!hasBranch())return null;
  const result=await supabaseClient.rpc('tkn_v5262_cashier_box_sale_access',{p_shift_id:shift.shift_id});
  if(result.error){console.warn('Cashier box-sale access unavailable:',result.error.message);return null}
  boxSaleAccess=Array.isArray(result.data)?result.data[0]:result.data;
  if(boxSaleAccess?.branch_id&&boxSaleAccess.branch_id!==E.branch.value){boxSaleAccess=null;return null}
  if(E.boxSaleButton)E.boxSaleButton.hidden=!canBoxSale();
  window.dispatchEvent(new CustomEvent('tkn:box-sale-access',{detail:boxSaleAccess||null}));
  return boxSaleAccess;
}
function openBoxSale(scan=''){
  if(!shift?.shift_id)return msg(E.actionMsg,'กรุณาเปิดกะด้วยรหัสพนักงานก่อน','error');
  if(!canBoxSale())return msg(E.actionMsg,'รหัสพนักงานนี้ไม่มีสิทธิ์ขายยกกล่อง','error');
  window.dispatchEvent(new CustomEvent('tkn:box-sale-open',{detail:{scan,shift_id:shift.shift_id,branch_id:E.branch.value,access:boxSaleAccess}}));
}
window.TKNPosBoxSaleContext={
  get:()=>({shift_id:shift?.shift_id||null,branch_id:hasBranch()?E.branch.value:null,access:boxSaleAccess,cashier:shift?{employee_code:shift.employee_code,display_name:shift.display_name}:null}),
  refresh:refreshBoxSaleAccess,
  open:openBoxSale
};
...
const VAT_RATE=7;
const VAT_MODE_KEY='tkn_pos_vat_mode';
...
function vatMode(){return document.querySelector('input[name="vatMode"]:checked')?.value||'INCLUDED'}
function baseNet(){return Math.max(roundMoney(subtotal()-itemDiscountTotal()-discount()),0)}
function checkoutPricing(){
  const mode=vatMode();
  const base=baseNet();
  if(mode==='NONE')return{mode,rate:0,base,vat:0,total:base,payloadFactor:1};
  if(mode==='INCLUDED'){
    const vat=roundMoney(base*VAT_RATE/(100+VAT_RATE));
    return{mode,rate:VAT_RATE,base:roundMoney(base-vat),vat,total:base,payloadFactor:1};
  }
  const factor=1+(VAT_RATE/100);
  const grossSubtotal=roundMoney([...cart.values()].reduce((sum,item)=>sum+(number(item.qty)*roundMoney(number(item.price)*factor)),0));
  const grossItemDiscount=roundMoney([...cart.values()].reduce((sum,item)=>sum+roundMoney(lineDiscount(item)*factor),0));
  const grossBillDiscount=roundMoney(discount()*factor);
  const total=Math.max(roundMoney(grossSubtotal-grossItemDiscount-grossBillDiscount),0);
  return{mode,rate:VAT_RATE,base,vat:Math.max(roundMoney(total-base),0),total,payloadFactor:factor};
}
function net(){return checkoutPricing().total}
function vatModeLabel(mode=vatMode()){
  return mode==='NONE'?'ปิด VAT':mode==='EXCLUDED'?'VAT นอก 7%':'VAT ใน 7%';
}
function restoreVatMode(){
  const saved=localStorage.getItem(VAT_MODE_KEY);
  const allowed=['NONE','INCLUDED','EXCLUDED'];
  const mode=allowed.includes(saved)?saved:'INCLUDED';
  const radio=document.querySelector(`input[name="vatMode"][value="${mode}"]`);
  if(radio)radio.checked=true;
}
function setVatMode(mode){
  if(!['NONE','INCLUDED','EXCLUDED'].includes(mode))return;
  localStorage.setItem(VAT_MODE_KEY,mode);
  updateTotals();
  if(E.paymentDialog?.open){E.paymentDialogNet.textContent=money(net());configurePaymentFields();}
}
function refreshPosAvailability(status=''){
  ...
  document.querySelectorAll('input[name="vatMode"]').forEach(r=>{r.disabled=!canWork});
...
async function loadBranches(){
  ...
  const requestedBranch=new URLSearchParams(location.search).get('branch')||sessionStorage.getItem('tkn_inventory_branch_id')||'';
  E.branch.value=rows.some(b=>b.id===requestedBranch)
    ? requestedBranch
    : rows.some(b=>b.id===access?.branch_id) ? access.branch_id : rows[0].id;
  if(!hasBranch()){
...
async function init(){
  ...
  if(hasBranch())sessionStorage.setItem('tkn_inventory_branch_id',E.branch.value);
...
async function searchProducts(event){
  ...
  if(scan.kind==='BOX'){
    if(!shift?.shift_id)return msg(E.searchMsg,'กรุณาเปิดกะด้วยรหัสพนักงานก่อนสแกนกล่อง','error');
    if(!canBoxSale())return msg(E.searchMsg,'รหัสพนักงานที่เปิดกะไม่มีสิทธิ์ขายยกกล่อง','error');
    openBoxSale(scan.raw);
    E.search.value='';
    return;
  }
  ...
  try{
    ...
    if(ids.length){
      // v5.26.1: หักเฉพาะกล่องที่อยู่คลังสาขาปัจจุบัน และคงพฤติกรรมเดิมสำหรับกล่องเก่าที่ยังไม่ระบุสาขา
      let boxed=await supabaseClient.from('tkn_v5261_box_item_locations')
        .select('product_id,quantity,branch_id,location_state').in('product_id',ids).gt('quantity',0);
      if(boxed.error && /does not exist|schema cache|relation/i.test(boxed.error.message||'')){
        boxed=await supabaseClient.from('stock_box_items').select('product_id,quantity,stock_boxes!inner(status)').in('product_id',ids).in('stock_boxes.status',['DRAFT','CLOSED']).gt('quantity',0);
      }
      if(boxed.error)throw new Error(`ตรวจสต็อกหน้าร้านไม่สำเร็จ: ${boxed.error.message}`);
      (boxed.data||[]).forEach(item=>{
        const tracked=('location_state' in item);
        const shouldReserve=!tracked || (item.location_state==='WAREHOUSE' && (!item.branch_id || item.branch_id===E.branch.value));
        if(shouldReserve)boxedMap.set(item.product_id,number(boxedMap.get(item.product_id))+number(item.quantity));
      });
    }
...
function updateTotals(){
  ...
  if(E.itemDiscountTotal)E.itemDiscountTotal.textContent=money(itemDiscountTotal());
  if(E.vatBaseAmount)E.vatBaseAmount.textContent=money(pricing.base);
  if(E.vatAmount)E.vatAmount.textContent=money(pricing.vat);
  if(E.vatModeHelp)E.vatModeHelp.textContent=pricing.mode==='NONE'?'ปิด VAT · ไม่คิดภาษีในบิลนี้':pricing.mode==='EXCLUDED'?'VAT นอก 7% · ระบบบวก VAT เพิ่มจากยอดหลังส่วนลด':'VAT ใน 7% · ราคาที่เห็นรวม VAT แล้ว';
  if(E.vatAmountLabel)E.vatAmountLabel.textContent=pricing.mode==='NONE'?'VAT':'VAT 7%'+(pricing.mode==='INCLUDED'?' (รวมในราคา)':' (บวกเพิ่ม)');
  if(E.vatBaseLabel)E.vatBaseLabel.textContent=pricing.mode==='INCLUDED'?'มูลค่าก่อน VAT':'ฐานก่อน VAT';
  E.netTotal.textContent=money(pricing.total);
  if(E.paymentDialog?.open)E.paymentDialogNet.textContent=money(pricing.total);
  refreshPosAvailability();
}
...
async function checkout(event){
  ...
  const pricing=checkoutPricing();
  const total=pricing.total,cash=E.payment.value==='CASH',received=cash?number(E.paymentDialogReceived.value):total;
  const factor=pricing.payloadFactor;
  const items=[...cart.values()].map(x=>({
    product_id:x.id,
    quantity:x.qty,
    unit_price:pricing.mode==='EXCLUDED'?roundMoney(number(x.price)*factor):number(x.price),
    discount_amount:pricing.mode==='EXCLUDED'?roundMoney(lineDiscount(x)*factor):lineDiscount(x)
  }));
  const billDiscount=pricing.mode==='EXCLUDED'?roundMoney(discount()*factor):discount();
  ...
  E.confirmPayment.disabled=true;msg(E.actionMsg,`กำลังบันทึกการขาย · ${vatModeLabel(pricing.mode)}...`);
  let result=await supabaseClient.rpc('create_pos_sale_safety_v3_5',{p_branch_id:E.branch.value,p_items:items,p_discount_amount:billDiscount,p_payment_method:E.payment.value,p_received_amount:received,p_customer_name:E.customerName.value.trim()||null,p_customer_phone:E.customerPhone.value.trim()||null,p_notes:saleNotes,p_cashier_shift_id:shift.shift_id,p_vat_rate:pricing.rate});
  if(result.error&&/create_pos_sale_safety_v3_5|schema cache|function/i.test(result.error.message||'')){
    if(pricing.mode==='EXCLUDED'){
      checkoutSubmitting=false;E.confirmPayment.disabled=false;
      return msg(E.actionMsg,'VAT นอกต้องใช้ POS Safety RPC เพื่อให้ยอดและ VAT ตรงกัน ระบบจึงหยุดรายการนี้โดยไม่สร้างบิล','error');
    }
    console.warn('POS Safety RPC unavailable; using temporary legacy fallback.',result.error);
    result=await supabaseClient.rpc('create_pos_sale',{p_branch_id:E.branch.value,p_items:items,p_discount_amount:billDiscount,p_payment_method:E.payment.value,p_received_amount:received,p_customer_name:E.customerName.value.trim()||null,p_customer_phone:E.customerPhone.value.trim()||null,p_notes:saleNotes});
  }
  ...
  await writeAudit('POS_VAT_MODE','SALE',result.data.sale_no,'รูปแบบ VAT ของบิล',{sale_no:result.data.sale_no,vat_mode:pricing.mode,vat_rate:pricing.rate,vat_base:pricing.base,vat_amount:pricing.vat,net_total:pricing.total});
...
function heldPayload(){return {branch:E.branch.value,payment:E.payment.value,customerName:E.customerName.value,customerPhone:E.customerPhone.value,discount:E.discount.value,vatMode:vatMode(),notes:E.notes.value,items:[...cart.values()]}}
function applyHeldPayload(p){if(validUuid(p.branch))E.branch.value=p.branch;E.payment.value=p.payment||'CASH';E.customerName.value=p.customerName||'';E.customerPhone.value=p.customerPhone||'';E.discount.value=p.discount||0;if(['NONE','INCLUDED','EXCLUDED'].includes(p.vatMode)){const radio=document.querySelector(`input[name="vatMode"][value="${p.vatMode}"]`);if(radio)radio.checked=true;}E.notes.value=p.notes||'';cart.clear();for(const x of p.items||[])cart.set(x.id,x);renderCart()}
async function hold(){
...
async function closeShift(event){
  ...
  const branchLabel=E.branch.options[E.branch.selectedIndex]?.text||'';
  shift=null;cashier=null;boxSaleAccess=null;saveShiftState();
  if(E.boxSaleButton)E.boxSaleButton.hidden=true;
  E.closeShiftDialog.close();
  clearOrder();
  E.closingCash.value='';
  ...
  const detail={
    shift:{...closedShift},
    result:{...(r.data||{})},
    branch_id:closedShift.branch_id||E.branch.value||null,
    branch_label:branchLabel,
    closing_cash_count:closingCashCount,
    closing_notes:closingNotes,
    closed_at:closedAt
  };
  window.dispatchEvent(new CustomEvent('tkn:shift-closed',{detail}));

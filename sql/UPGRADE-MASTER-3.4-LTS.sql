-- MASTER 3.4 LTS RC
-- Adds custom date-range report without changing existing tables.
begin;
...
begin
  if p_start_date is null or p_end_date is null then
    raise exception 'กรุณาระบุวันที่เริ่มต้นและวันที่สิ้นสุด';
  end if;

  if p_end_date < p_start_date then
    raise exception 'วันที่สิ้นสุดต้องไม่น้อยกว่าวันที่เริ่มต้น';
  end if;

  if p_end_date - p_start_date > 366 then
    raise exception 'ช่วงรายงานต้องไม่เกิน 366 วัน';
  end if;

  if not (
    public.user_has_permission('report.view'::text,auth.uid())
    or public.user_has_permission('dashboard.view'::text,auth.uid())
    or public.user_has_permission('dashboard.branch_view'::text,auth.uid())
  ) then
    raise exception 'ไม่มีสิทธิ์ดูรายงาน';
  end if;

  v_start := p_start_date::timestamp;
  v_end := (p_end_date + 1)::timestamp;
  ...
  items as (
    ...
    from (
      select rb.sale_id,rb.product_id,rb.product_code,rb.barcode,
        rb.product_name,rb.sold_quantity,rb.returned_quantity,
        rb.returnable_quantity,rb.unit_price,
  ...
  select jsonb_build_object(
    'period',jsonb_build_object(
      'type','RANGE','start',v_start,'end',v_end
    ),
...
revoke all on function public.get_sales_control_dashboard_range_v3_4(
  date,date,uuid,integer
) from public;

grant execute on function public.get_sales_control_dashboard_range_v3_4(
  date,date,uuid,integer
) to authenticated;
